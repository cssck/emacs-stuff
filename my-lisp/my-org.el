;;; my-org.el --- Tweaks for my org-mode configurations -*- lexical-binding: t -*-
...
(defvar my-org--capture-coach-person-history nil)

(declare-function message-fetch-field "message" (header &optional first))
(declare-function notmuch-show-get-header "notmuch-show")

(defun my-org--capture-coach-person-message-from ()
  "Return default value for `my-org--capture-coach-person-prompt'."
  (when-let* ((from (cond
...
(defun my-org--capture-coach-person-message-from-and-subject ()
  "Return default value for `my-org--capture-coach-person-prompt'."
  (cond
   ((derived-mode-p 'message-mode)
...
(defun my-org--capture-coach-person-prompt ()
  "Prompt for person for use in `my-org-capture-coach'."
  (completing-read "Person to coach: "
                   my-org--capture-coach-person-history
                   nil nil nil
                   'my-org--capture-coach-person-history
                   (my-org--capture-coach-person-message-from)))

(defvar my-org--capture-coach-description-history nil)

(defun my-org--capture-coach-description-prompt ()
  "Prompt for description in `my-org-capture-coach'."
  (read-string "Description: "
               nil
               'my-org--capture-coach-description-history
               (my-org--capture-coach-person-message-from-and-subject)))

(defun my-org--capture-coach-date-prompt-range ()
  "Prompt for Org date and return it as a +1h range.
For use in `my-org-capture-coach'."
...
%%a%%?"
            (my-org--capture-coach-person-prompt)
            (my-org--capture-coach-description-prompt)
            identifier
            identifier)))

(defun my-org-capture-coach-clock ()
...
(defun my-org--capture-no-delete-windows (&rest args)
  "Apply ARGS while ignoring `delete-other-windows'."
  (cl-letf (((symbol-function 'delete-other-windows) 'ignore))
    (apply args)))

;; Same source as above
(advice-add 'org-capture-place-template :around 'my-org--capture-no-delete-windows)
(advice-add 'org-add-log-note :around 'my-org--capture-no-delete-windows)

;;;;; Custom function to select a project to add to

(defun my-org--get-outline (&optional file)
...
(defvar my-org-outline-history nil
  "Minibuffer history for `my-org-outline-prompt'.")

(defun my-org-outline-prompt (&optional file)
  "Prompt for outline among headings retrieved by `my-org--get-outline'.
With optional FILE use the outline of it, otherwise use that of
the current file."
  (let ((current-file (or file buffer-file-name))
        (default (car my-org-outline-history)))
    (completing-read
     (format-prompt
      (format "Select heading inside `%s': "
              (propertize (file-name-nondirectory current-file) 'face 'error))
      default)
     (my-common-completion-table-no-sort 'imenu (my-org--get-outline current-file))
     nil :require-match nil 'my-org-outline-history default)))

(defvar my-org-file-history nil
  "Minibuffer history of `my-org-file-prompt'.")

(defun my-org--not-useful-p (file)
...
(defun my-org-file-prompt ()
  "Select a file in the `org-directory'."
  (if-let* ((dir org-directory)
            (files (directory-files-recursively org-directory ".*" nil))
            (files (seq-remove #'my-org--not-useful-p files)))
      (let ((default (car my-org-file-history)))
        (completing-read
         (format-prompt "Select file" default)
         (my-common-completion-table 'file files)
         nil :require-match nil 'my-org-file-history default))
    (user-error "There are no files in the `org-directory'")))

;;;###autoload
(defun my-org-select-heading-in-file ()
...
there."
  ...
  (interactive)
  (pcase-let* ((file (my-org-file-prompt))
               (line-with-heading (my-org-outline-prompt file))
...
(defalias 'my-org-goto-heading-in-file 'my-org-select-heading-in-file
 "Alias for `my-org-select-heading-in-file'.")

;;;; Org clock report

(defvar my-org-clock--template-with-effort
...
(defvar my-org-clock--template-no-effort
...
(defvar my-org-clock--report-range-history nil
  "Minibuffer history for `my-org-clock--report-range-prompt'.")

(defun my-org-clock--report-range-prompt ()
  "Prompt for a clock table range among `my-org-clock--ranges'."
  (let ((default (car my-org-clock--report-range-history)))
    (completing-read
     (format-prompt "Select a time range for the clock" default)
     my-org-clock--ranges nil :require-match nil 'my-org-clock--report-range-history
     default)))

(defun my-org-clock--get-report (scope)
...
use the entire file's contents.  Else use those of the current subtree."
  (let ((buffer (get-buffer-create "*my-org-custom-clock-report*")))
    (save-restriction
      (unwind-protect
          (progn
            (pcase scope
            ...
            (let ((contents (buffer-substring (point-min) (point-max))))
              (with-current-buffer buffer
                ...
                (org-mode)
                (save-excursion
                  (insert (format "%s\n\n" my-org-clock--template-with-effort))
                  (insert contents))
                (save-excursion
                  (let ((range (my-org-clock--report-range-prompt)))
...
;;;###autoload
(defun my-org-clock-report-current-subtree-or-file (&optional whole-buffer)
...
When called interactively WHOLE-BUFFER is a prefix argument."
  (interactive "P")
  (when-let* ((buffer (my-org-clock--get-report (if whole-buffer 'file 'subtree))))
    (pop-to-buffer buffer)))

;;;###autoload
(defun my-org-clock-select-heading-and-clock-report ()
  "Select a heading in a file and do a clock report for it in a new buffer."
  (interactive)
  (call-interactively 'my-org-select-heading-in-file)
  (call-interactively 'my-org-clock-report-current-subtree-or-file))

;;;;; Coaching-related Org custom clocking

;; TODO 2024-12-15: This sort of thing must exist in Org, but I did
;; not find it.
(defun my-org--timestamp-to-time (string)
  "Return time object of STRING timestamp."
  (org-timestamp-to-time (org-timestamp-from-string string)))

(defun my-org-coach--get-entries (todo-keyword string since)
            ...
            (org-map-entries
             (lambda ()
               (when-let* ((case-fold-search t)
                           (started (my-org--timestamp-to-time (or (org-entry-get nil "DEADLINE") (org-entry-get nil "SCHEDULED"))))
                           (closed (my-org--timestamp-to-time (org-entry-get nil "CLOSED")))
...
(defvar my-org-coach--name-history nil
  "Minibuffer history of `my-org-coach--name-prompt'.")

(defun my-org-coach--name-prompt ()
  "Prompt for name of person."
  (let ((default (car my-org-coach--name-history)))
    (read-string
     (format-prompt "Name of person" default)
     nil 'my-org-coach--name-history default)))

;;;###autoload
(defun my-org-coach-report (name since)
  "Produce clock report for coaching with person of NAME.
SINCE is the date (of time 00:00) to count from until now."
  (interactive
   (list
    (my-org-coach--name-prompt)
    (format "[%s]" (org-read-date))))
  (if-let* ((since-object (my-org--timestamp-to-time since))
            (entries (my-org-coach--get-entries "done" name since-object))
            (buffer (get-buffer-create "*my-org-coach-entries*")))
      (with-current-buffer (pop-to-buffer buffer)
        ...
        (goto-char (point-min))
        (save-excursion
          (insert (format "%s\n\n" my-org-clock--template-no-effort)))
        (save-excursion
          (goto-char (line-end-position))
...
(defun my-org-agenda-include-priority-no-timestamp ()
...
(defvar my-org-custom-daily-agenda
  ...
  `((tags-todo "*"
               ((org-agenda-overriding-header "Important tasks without a date\n")
                ...
                ;; `my-org-agenda-include-priority-no-timestamp'.
                ;;
                ;; (org-agenda-skip-function '(org-agenda-skip-subtree-if nil '(timestamp)))
                ;; (org-agenda-skip-function
                ;;  `(org-agenda-skip-entry-if
                ;;    'notregexp ,(format "\\[#%s\\]" (char-to-string org-priority-highest))))
                (org-agenda-skip-function #'my-org-agenda-include-priority-no-timestamp)
...
(defvar my-org-agenda-after-edit-hook nil
  "Hook that runs after select Org commands.
To be used with `advice-add'.")

(defun my-org--agenda-after-edit (&rest _)
  "Run `my-org-agenda-after-edit-hook'."
  (run-hooks 'my-org-agenda-after-edit-hook))

(defvar my-org-after-deadline-or-schedule-hook nil
  "Hook that runs after `org--deadline-or-schedule'.
To be used with `advice-add'.")

(defvar my-org--appt-agenda-commands
  '( org-agenda-archive org-agenda-deadline org-agenda-schedule
     org-agenda-todo org-archive-subtree)
  "List of commands that run `my-org-agenda-after-edit-hook'.")

(dolist (fn my-org--appt-agenda-commands)
  (advice-add fn :after #'my-org--agenda-after-edit))

(defun my-org--after-deadline-or-schedule (&rest _)
  "Run `my-org-after-deadline-or-schedule-hook'."
  (run-hooks 'my-org-after-deadline-or-schedule-hook))

(defun my-org-org-agenda-to-appt ()
...
(dolist (hook '(org-capture-after-finalize-hook
                ...
                my-org-agenda-after-edit-hook))
  (add-hook hook #'my-org-org-agenda-to-appt))

(declare-function org--deadline-or-schedule "org" (arg type time))

(advice-add #'org--deadline-or-schedule :after #'my-org--after-deadline-or-schedule)

(add-hook 'my-org-after-deadline-or-schedule-hook #'my-org-org-agenda-to-appt)
...
;;;###autoload
(defun my-org-ox-html ()
  "Streamline HTML export."
  (interactive)
  (org-html-export-as-html nil nil nil t nil))

;;;###autoload
(defun my-org-ox-texinfo ()
...
(defun my-org--id-get-readable ()
  "Like `my-org--id-get' but use the heading wording to create and ID."
  (let* ((pos (point))
         (id (org-entry-get pos "CUSTOM_ID")))
    (or (and id (stringp id) (string-match-p "\\S-" id))
        (and (setq id (my-org--heading-to-id))
...
;;;###autoload
(defun my-org-id-headlines ()
  "Add missing CUSTOM_ID to all headlines in current file."
  (interactive)
  (org-map-entries (lambda () (my-org--id-get))))

;;;###autoload
(defun my-org-id-headlines-readable ()
  "Like `my-org-id-headlines' but with readable IDs.
A readable identifier is one derived from the text of the heading.  In
theory, this may not be unique."
  (interactive)
  (org-map-entries (lambda () (my-org--id-get-readable))))

;;;###autoload
(defun my-org-id-headline (&optional readable)
  "Add missing CUSTOM_ID to headline at point.
With optional prefix argument READABLE get a readable identifier derived
from the heading text instead of a UUID."
  (interactive "P")
  (funcall (if readable 'my-org--id-get-readable 'my-org--id-get)))

;;;###autoload
(defun my-org-get-dotemacs-link ()
  "Get URL to current heading in my dotemacs file."
  (declare (interactive-only t))
  (interactive)
  (if (and (derived-mode-p 'org-mode)
           (string-match-p "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs\\.org\\'" buffer-file-name))
