;;; my-search.el --- Extensions to isearch, replace, grep for my dotemacs -*- lexical-binding: t -*-
...
(defcustom my-search-outline-regexp-alist
  '((emacs-lisp-mode . "^\\((\\|;;;+ \\)")
...
To be used by `my-search-occur-outline'."
  :type 'alist
  :group 'my-search)

(defcustom my-search-todo-keywords
...
;;;###autoload
(defun my-search-isearch-other-end ()
...
;;;###autoload
(defun my-search-isearch-abort-dwim ()
...
;;;###autoload
(defun my-search-isearch-repeat-forward (&optional arg)
...
;;;###autoload
(defun my-search-isearch-repeat-backward (&optional arg)
...
(defmacro my-search-isearch-occurrence (name edge &optional doc)
...
(my-search-isearch-occurrence
 my-search-isearch-beginning-of-buffer
 "beginning"
...
(my-search-isearch-occurrence
 my-search-isearch-end-of-buffer
 "end"
...
(defvar my-search-markup-replacements
  '((elisp-to-org-code "`\\(.*?\\)'" "~\\1~")
...
(defvar my-search--replace-markup-history '()
  "Minibuffer history of `my-search-replace-markup'.")

(defun my-search--replace-markup-prompt ()
  "Prompt for `my-search-replace-markup'."
  (let* ((def (nth 0 my-search--replace-markup-history))
         (prompt (if def
    ...
    (intern
     (completing-read
      ...
      my-search-markup-replacements
      nil t nil 'my-search--replace-markup-history def))))

(defun my-search-replace-markup (type)
  "Perform TYPE of markup replacement.
TYPE is the car of a list in `my-search-markup-replacements'.
...
boundaries, else start from point to the end of the buffer."
  (interactive (list (my-search--replace-markup-prompt)))
  (if-let* ((types my-search-markup-replacements)
...
(defun my-search-occur-urls ()
  ...
  (let ((buf-name (format "*links in <%s>*" (buffer-name))))
    ...
    (occur-1 my-common-url-regexp "\\&" (list (current-buffer)) buf-name)
    (remove-hook 'occur-hook #'goto-address-mode)))

;;;###autoload
(defun my-search-occur-browse-url ()
  "Point browser at a URL in the buffer using completion.
Which web browser to use depends on the value of the variable
`browse-url-browser-function'.

Also see `my-search-occur-urls'."
  (interactive)
  (let ((matches nil))
    (save-excursion
      (goto-char (point-min))
      (while (search-forward-regexp my-common-url-regexp nil t)
        (push (match-string-no-properties 0) matches)))
    (funcall browse-url-browser-function
             (completing-read "Browse URL: " matches nil t))))

(defvar my-search--occur-outline-hist '()
  "Minibuffer history of `my-search-occur-outline'.")

(defun my-search--occur-outline-prompt ()
  "Helper prompt for `my-search-occur-outline'."
  (let* ((alist my-search-outline-regexp-alist)
         (key (car (assoc major-mode alist)))
         (default (or key (nth 1 my-search--occur-outline-hist))))
    (completing-read
     (format "Outline style [%s]: " default)
     (mapcar #'car alist)
     nil nil nil 'my-search--occur-outline-hist default)))

(defvar-local my-search--remap-cookie nil
  "Current local value of `my-search--remap-match-face'.")

(defface my-search-match '((t :inherit default))
  "Face intended to override `match' buffer-locally.")

(defun my-search--remap-match-face (buf)
  "Remap `match' to `my-search-match' in BUF."
  (with-current-buffer buf
    (setq my-search--remap-cookie
          (face-remap-add-relative 'match 'my-search-match))))

;;;###autoload
(defun my-search-occur-outline (&optional arg)
  "Produce buffer outline from `my-search-outline-regexp-alist'.

With optional prefix ARG (\\[universal-argument]), prompt for a
preset among the entries in `my-search-outline-regexp-alist'.

ARG may also be a string (or regular expression) when called from
Lisp."
  (interactive "P")
  (let* ((regexp (when (and arg (not (stringp arg)))
                   (my-search--occur-outline-prompt)))
         (rx (cond
              ((stringp arg)
               arg)
              ((and arg (string= major-mode regexp))
               (alist-get regexp my-search-outline-regexp-alist))
              ((assoc major-mode my-search-outline-regexp-alist)
               (alist-get major-mode my-search-outline-regexp-alist))
    ...
    (my-search--remap-match-face buf-name)
    (add-to-history 'my-search--occur-outline-hist regexp)))

;;;###autoload
(defun my-search-occur-todo-keywords (&optional context)
  "Produce Occur buffer with `my-search-todo-keywords'.
...
Also see `my-search-grep-todo-keywords'."
  (interactive "P")
  (let* ((case-fold-search nil)
    ...
    (occur-1 my-search-todo-keywords num (list (current-buffer)) buf-name)))

;;;; Outline

(defun my-search--get-outline ()
...
(defun my-search--outline-prompt ()
  "Prompt for outline among headings retrieved by `my-search--get-outline'."
  (completing-read
   "Go to outline: "
   (my-common-completion-table-no-sort 'imenu (my-search--get-outline))
   nil :require-match))

(defvar my-search-outline-hook nil
  "Normal hook to run at the end of `my-search-outline'.")

;;;###autoload
(defun my-search-outline ()
  "Go to the line of the given outline using completion."
  (interactive)
  (when-let* ((selection (my-search--outline-prompt))
              (line (string-to-number (car (split-string selection "\t")))))
    (goto-line line)
    (run-hooks 'my-search-outline-hook)))

;;;; Grep

(defvar my-search--grep-hist nil
  "Input history of grep searches.")

(defun my-search-grep-prompt (&optional recursive)
...
recursively."
  (read-regexp
   (concat (if recursive
             ...
             "Local")
           " grep for PATTERN: ")
   nil 'my-search--grep-hist))

;;;###autoload
(defun my-search-grep (regexp &optional recursive)
  "Run grep for REGEXP.
...
search starting from the current directory with `rgrep'."
  (interactive
   (list
    (my-search-grep-prompt current-prefix-arg)
    current-prefix-arg))
  (unless grep-command
...
;;;###autoload
(defun my-search-grep-todo-keywords (&optional arg)
  "Use `my-search-grep' to find `my-search-todo-keywords'.

With optional prefix ARG use git-grep instead for the entire
repository (runs `my-search-git-grep-todo-keywords').  If Git
is not available on the system, run `my-search-grep'
recursively, starting from the current directory.

Also see `my-search-occur-todo-keywords'."
  (interactive "P")
  (cond
   (arg
    (if (executable-find "git")
        (my-search-git-grep-todo-keywords)
      (my-search-grep my-search-todo-keywords t)))
   (t
    (my-search-grep my-search-todo-keywords))))
...
;;;###autoload
(defun my-search-git-grep-todo-keywords ()
  "Use the git-grep mechanism for `my-search-todo-keywords'."
  (interactive)
  (let ((regexp my-search-todo-keywords)
    ...
    (compilation-start
     ...
     'grep-mode
     (lambda (mode) (format "*my-search-git-%s for '%s'" mode regexp))
     t)))

(defun my-search--add-revert-function (buffer mode fn regexp)
  "Append `revert-buffer-function' for FN with REGEXP to MODE BUFFER variables.
See `my-search-find-grep-buffer' (or related) for the kind of
...
(defun my-search--start-compilation (args mode buffer command query)
  ...
  (compilation-start
   ...
   (lambda (_mode) buffer)
   :highlight-regexp)
  (my-search--add-revert-function buffer mode command query))

(defvar my-search--find-grep-hist '()
  "Minibuffer history for `my-search-find-grep-buffer' and related.")

(defmacro my-search-make-search (command docstring prompt function mode)
...
and function as links to the context they reference."
     ...
     (interactive
      (list
       (read-regexp ,prompt nil 'my-search--find-grep-hist)))
     (let ((args (,function query))
           (buffer-name (format "*my-search-find for '%s'*" query)))
       (my-search--start-compilation args ,mode buffer-name ',command query))))

(defun my-search--find-grep-args (regexp)
...
;;;###autoload (autoload 'my-search-find-grep-buffer "my-search")
(my-search-make-search
 my-search-find-grep-buffer
 "Combine find with grep to produce a buffer for REGEXP matches"
 "Find files matching REGEXP and show a grep buffer: "
 my-search--find-grep-args
 "grep")

(defun my-search--find-grep-files-args (regexp)
...
;;;###autoload (autoload 'my-search-find-grep-files-buffer "my-search")
(my-search-make-search
 my-search-find-grep-files-buffer
 ...
 my-search--find-grep-files-args
 "dired")

(defun my-search--find-file-names-args (regexp)
...
;;;###autoload (autoload 'my-search-find-files-buffer "my-search")
(my-search-make-search
 my-search-find-files-buffer
 "Use find to produce a buffer for file names matching REGEXP"
 "Find files with name matching REGEXP and show a file listing: "
 my-search--find-file-names-args
 "dired")

;; (defun my-search-find-grep-file (regexp)
;;   "Use find to produce list of files that include REGEXP."
;;   (interactive
;;    (list
;;     (read-regexp "Find and grep for REGEXP: " nil 'my-search--find-grep-hist)))
