;;; my-eww.el --- Extensions for EWW -*- lexical-binding: t -*-
...
(defun my-eww-buffer-url-prompt ()
  "Prompt for a url in the current buffer."
  (when-let* ((completion-extra-properties `(:annotation-function ,#'my-eww--annotate-with-url))
              (link-data (my-eww--get-urls))
              (candidates (mapcar
                           (pcase-lambda (`(,position ,name ,_))
                             (format "%s	%s" position name))
                           link-data))
              (table (my-common-completion-table-no-sort nil candidates))
              (selection
               (completing-read
...
(defun my-eww-visit-url-on-page (&optional new-buffer)
...
buffer instead of the current one."
  ...
  (if-let* ((data (my-eww-buffer-url-prompt))
            (url (nth 2 data)))
      (eww url new-buffer)
    (error "Cannot find URL in data `%s'" data)))

(defun my-eww-jump-to-url-on-page ()
...
(defvar my-eww--occur-feed-regexp
  (concat "\\(rss\\|atom\\)\\+xml.\\(.\\|\n\\)"
          ".*href=[\"']\\(.*?\\)[\"']")
  "Regular expression to match web feeds in HTML source.")

(defun my-eww-find-feed ()
  ...
  (let* ((url (or (plist-get eww-data :start)
    ...
    (with-temp-buffer
      (insert source)
      (occur-1 my-eww--occur-feed-regexp "\\3" (list (current-buffer)) buf-name))
      ...
      (with-current-buffer (get-buffer buf-name)
        (let ((inhibit-read-only t)
          ...
          (while (< (point) (point-max))
            ...
            (when (and (looking-at my-common-url-regexp)
                       (not (looking-at (format "%s.*" url))))
              (insert base-url))
            (forward-line 1)))))))

;;;###autoload
(defun my-eww-open-in-other-window ()
