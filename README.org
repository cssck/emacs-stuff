...
** /etc/guix-systems/normandy.scm

Let's start by adding my custom modules. This isn't exactly clean but it is functional.
#+BEGIN_SRC scheme :tangle "~/git/emacs-stuff/configs-root/guix/etc/guix-systems/normandy.scm"
...
** /etc/guix-modules/aprilsnow/packages

Let's start with the packages first.

*** brogue.scm
#+BEGIN_SRC scheme :tangle "~/git/emacs-stuff/configs-root/guix-modules/etc/guix-modules/aprilsnow/packages/brogue.scm"
...
#+BEGIN_SRC scheme :tangle "~/git/emacs-stuff/configs-root/guix-modules/etc/guix-modules/aprilsnow/packages/distrobox-docker.scm"
...
#+BEGIN_SRC scheme :tangle "~/git/emacs-stuff/configs-root/guix-modules/etc/guix-modules/aprilsnow/packages/emulators.scm"
...
#+begin_src scheme :tangle "~/git/emacs-stuff/configs-root/guix-modules/etc/guix-modules/aprilsnow/packages/gnome.scm"
...
#+begin_src scheme :tangle ~/git/emacs-stuff/configs-root/guix-modules/etc/guix-modules/aprilsnow/packages/throttled.scm
...
#+end_src
** /etc/guix-modules/aprilsnow/services
...
#+BEGIN_SRC scheme :tangle "~/git/emacs-stuff/configs-root/guix-modules/etc/guix-modules/aprilsnow/services/mount-rshared.scm"
...
#+BEGIN_SRC scheme :tangle "~/git/emacs-stuff/configs-root/guix-modules/etc/guix-modules/aprilsnow/services/virsh.scm"
...
#+begin_src scheme :tangle "~/git/emacs-stuff/configs-root/guix-modules/etc/guix-modules/aprilsnow/services/throttled.scm"
...
*** Goals
My main goal for this config is to have a final home in Emacs. I noticed that when I try writing my config in plain code I tend to treat is as disposable and it doesn't stick well. It is easy to get lost and it's hard to place new code where it should go.

I want a configuration that's going to be my planner, reader, writer, library keeper. I plan to incorporate Emacs Writing Studio into this config as a module.

There's two parts to a config:
- Technical configuration
- Workflow configuration

I mostly only bothered with the technical part, but I didn't touch my workflow, till now.
I'm taking inspiration from Sacha Chua's and Prot's emacs setup 
- [[https://sachachua.com/dotemacs/index.html][Sacha Chua's Emacs configuration]]
- https://protesilaos.com/emacs/dotemacs
*** Why?
I used Doom Emacs before, it was cool but unwieldy. I remember trying to change the org mode font for two days and it turned out to be a doom function. I want to use more of built in stuff.
...
*** The =early-init.el= debugs on error
From [[https://github.com/jamescherti/minimal-emacs.d]]
During the development of your init files, the author strongly recommends adding the following line at the very beginning of your ~/.emacs.d/early-init.el file:

#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/early-init.el"
...
*** The =early-init.el= and =init.el= compiles Elisp code natively and automatically

Why use =compile.el=? Because you are likely running a significant amount of interpreted,
slow Elisp code that Emacs did not compile automatically. Ensuring
that Elisp is native-compiled significantly improves Emacs'
performance. Unfortunately, functions like package-install and
package-recompile-all do not compile .el files that were not installed
using package.el. Since these files are not byte-compiled, the Emacs
JIT compiler does not native-compile them either, as a byte-compiled
file signals the JIT compiler to perform native compilation. In
contrast, compile-angel modes ensure that all loaded .el files are
compiled transparently, regardless of whether they are part of a
package.
...
the ~my-emacs-with-desktop-session~ macro that I described above to
...
*** The =early-init.el= initializes the package cache
...
- The =my-emacs-modus-themes.el= module
- The =my-emacs-ef-themes.el= module
- The =my-emacs-standard-themes.el= module
...
*** The =early-init.el= gives a name to the default theme
...
([[#h:77e4f174-0c86-460d-8a54-47545f922ae9][The =my-emacs-window.el= section about ~beframe~]]). The multi-frame
...
*** The =init.el= exwm toggle
EXWM can be toggled here. Its configuration is handled within a separate file.
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/init.el"
  (defvar use-exwm nil)

  (when use-exwm (load "~/.exwm.el"))
  
#+end_src
*** The =init.el= settings for ~lolsmacs~

There are countless custom Emacs distributions (including but not
limited to spacemacs, Prelude, Doom, scimax) and “better” default
style configurations (Better Defaults, and every init file ever) out
there and for good reason they all “scratch an itch”. Some of us
switch to those distributions and most of copy from them (if you don’t
you should) or both. The options and information can be overwhelming
you ask yourself “Where do I even start here? What is really important
and essential?” and the most important question “What are the
differences between objectively good features that we can all agree on
and features that are almost totally personal preference?” The truth
is that we can’t but we do have a very good way to move forward
anyway.

A UNIX guru once said that “There are no bad Emacs configurations,
only poor applications of them.” and I buy that. Magicians hide the
menu bar. Neophytes show line numbers. Speed demons use completion
skeletons and ido. Terminal junkies only use Control and Meta. GUI
people use word wrap. You get the picture. So it is hard to make a
generalized application everyone is different but they all share one
thing no matter the application: they’ve all suffered some serious
pain using text editors.

Pain is the best motivator for change in life. Shock horror sadness
and dismay follow a number of events using text editors. Emacs
included. Other times pain comes in a more diluted form of
disappointment. We’ve all exclaimed “What the heck?!” or the less
stressful “Really? Really?!” Common among all of them is their origin
the violation of the Law Of Least Surprise (LOLS).

The purpose of this lattice LOLSMacs tries to configure Emacs never to
break the LOLS.

P.S. it is a lattice not a configuration distribution or
initialization because all it does is establish some objective
boundaries for operation that let you stay happy and focus on more
important things.

#+begin_src emacs-lisp :tangle no
(use-package lolsmacs
  :vc (:url "https://github.com/grettke/lolsmacs")
(require 'lolsmacs)
(lolsmacs-init)
#+end_src
...
All user options must be set in a =my-emacs-pre-custom.el= file in
...
Prot is the developer/maintainer of three distinct theme packages. You
...
I normally use ~vertico~ with my own custom extensions to it ([[#h:3796a4c9-8659-4782-8aaa-3cf4e950927d][The =my-vertico.el= library]]),
...
- [[#h:22e97b4c-d88d-4deb-9ab3-f80631f9ff1d][The =my-emacs-completion.el= settings for ~consult~]]
- [[#h:61863da4-8739-42ae-a30f-6e9d686e1995][The =my-emacs-completion.el= section about ~embark~]]
...
Also check [[#h:ddb1070d-2f91-4224-ad43-ef03f038f787][the =my-emacs-which-key.el= module]].
...
Also check [[#h:4efde779-3ef0-40b7-82e4-70f71b321428][the =my-emacs-icons.el= module]].
...
#+begin_comment
I definitely need to learn the version control and magit and this is the opportunity.
#+end_comment
...
This sets up Emacs for me to be able to type in Ukrainian while still using Emacs key bindings involving modifier keys.
...
*** The =init.el= keymap for Dvorak

Emacs doesn't behave correctly when using Dvorak and an input method. To bypass this I add a keymap for Dvorak.
...
*** TODO The =init.el= setting to always start with the =*scratch*= buffer
...
#+begin_comment
I might try the agenda view by default actually
#+end_comment
...
- The =my-emacs-modules= directory :: This is where I store all the
  ...
  with =my-emacs-=, followed by a word that broadly describes their
  scope of application, such as ~my-emacs-font~, ~my-emacs-window~...
  ...
  the file name minus its file type extension: ~my-emacs-theme~ is
  the feature provided by =my-emacs-theme.el=.
...
- The =my-lisp= directory :: As with the aforementioned modules,
...
=my-emacs.org= file or its web page version. Still, this arrangement
...
*** The =init.el= setup for ~compile-angel~
...
*** The =init.el= macro to do nothing with Elisp code (~my-emacs-comment~)
...
~my-emacs-comment~ (though another is needed to also remove it and
...
*** The =init.el= macro to bind keys (~my-emacs-keybind~)
...
*** The =init.el= macro to define abbreviations (~my-emacs-abbrev~)
...
This is the same idea as ~my-emacs-keybind~, adjusted to work with
the ~define-abbrev~ function ([[#h:d3684ec0-a972-4033-a4d6-41ee16e891d9][The init.el macro to bind keys (~my-emacs-keybind~)]]).
...
those options must have the =my-emacs-pre-custom.el= file in place,
...
**** The =my-emacs-theme.el= section to load a theme (~my-emacs-load-theme-family~)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-theme.el" :mkdirp yes
...
This is one of the components of the =my-emacs-theme.el= module
([[#h:3f1a33e4-762e-4cf3-a0ae-4c2954d9cbb8][The =my-emacs-theme.el= section to load a theme (~my-emacs-load-theme-family~)]]).
...
Note that the ~my-emacs-comment~ is there for my testing purposes
([[#h:3b14faa6-83fd-4d5f-b3bc-85f72fd572d4][The =init.el= macro to do nothing with Elisp code (~my-emacs-comment~)]]).

#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-modus-themes.el"
;;; The Modus themes
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-ef-themes.el"
;;; The Ef (εὖ) themes
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-standard-themes.el"
;;; The Standard themes
...
**** The =my-emacs-theme.el= section for ~pulsar~
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-theme.el"
;;;; Pulsar
...
**** The =my-emacs-theme.el= section for ~lin~
:PROPERTIES:
...
**** The =my-emacs-theme.el= section for ~spacious-padding~
:PROPERTIES:
...
([[#h:41ee38d0-9099-4444-85e4-cc8dce00fb9a][The =my-emacs-modeline.el= module]]).
...
**** The =my-emacs-theme.el= section for ~rainbow-mode~
:PROPERTIES:
:CUSTOM_ID: h:9438236e-a8a4-45e0-8c61-8268c634d50b
:END:

This package produces an in-buffer preview of a colour value. I use
those while developing my themes, hence the ~my/rainbow-mode-in-themes~
...
**** The =my-emacs-theme.el= section for ~cursory~
:PROPERTIES:
...
**** The =my-emacs-theme.el= section for ~theme-buffet~
:PROPERTIES:
...
**** The =my-emacs-theme.el= section about ~fontaine~
:PROPERTIES:
...
([[#h:60d6aae2-6e4b-402c-b6a8-411fc49a6857][The =my-emacs-theme.el= section about ~variable-pitch-mode~ and font resizing]]).

Also check my ~show-font~ package ([[#h:60a005be-77bd-49f1-a865-78d7cf75bd2a][The =my-emacs-theme.el= section about ~show-font~]]).
...
**** The =my-emacs-theme.el= section about ~show-font~
:PROPERTIES:
...
To actually set fonts, I use my ~fontaine~ package ([[#h:cb41fef0-41a5-4a85-9552-496d96290258][The =my-emacs-theme.el= section about ~fontaine~]]).
...
**** The =my-emacs-theme.el= section about ~variable-pitch-mode~ and font resizing
:PROPERTIES:
...
families in my ~fontaine~ setup ([[#h:cb41fef0-41a5-4a85-9552-496d96290258][The =my-emacs-theme.el= section about ~fontaine~]]).
...
variable =my/enable-variable-pitch-in-hooks=. Exceptions to these
...
in the function ~my/enable-variable-pitch~.
...
**** The =my-emacs-theme.el= call to ~provide~
...
*** The =my-emacs-essentials.el= module
:PROPERTIES:
...
**** The =my-emacs-essentials.el= block with basic configurations
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-essentials.el" :mkdirp yes
...
**** The =my-emacs-essentials.el= section about =my-common.el= (custom basic functions)
:PROPERTIES:
:CUSTOM_ID: h:116854fa-2e31-4d23-ae6f-72f57ed571bc
:END:

[[#h:3fccfadf-22e9-457f-b9fd-ed1b48600d23][The =my-common.el= library]] contains custom snippets that I use in
...
Since the =my-common.el= defines some basic functions that can be
...
1. Truncate lines by default in a number of places and do not produce
   ...
   this, i.e. ~my-common-truncate-lines-silently~, may also be set
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-essentials.el"
...
**** The =my-emacs-essentials.el= section about =my-simple.el= (custom basic commands)
:PROPERTIES:
:CUSTOM_ID: h:f15bc843-2dd1-4792-95ec-9b7b6e561804
:END:

[[#h:5f78e837-0d27-4390-bd9a-6d0bca57fa50][The =my-simple.el= library]] is done in the same spirit as the
...
The ~my-simple-display-unsaved-buffers-on-exit~ produces a list of
...
**** The =my-emacs-essentials.el= section about =my-scratch.el= (scratch buffer per major mode)
:PROPERTIES:
:CUSTOM_ID: h:45b4fffa-2ce3-4119-b8c9-081a75135d98
:END:

[[#h:efebaec4-7273-4c5a-a4c4-9f99df0e46bb][The =my-scratch.el= library]] provides the means to create a scratch
...
buffer. Read the doc string of the command ~my-scratch-buffer~.
...
**** The =my-emacs-essentials.el= section about =my-pair.el= (insert character pairs)
:PROPERTIES:
:CUSTOM_ID: h:48d2bab5-241f-49dd-9554-8a4dcedce75e
:END:

[[#h:0a3f076d-386d-47e6-8af9-f17d02d507cf][The =my-pair.el= library]] defines a mechanism to insert character
pairs around the symbol at point or the active region. There is a user
option called ~my-pair-pairs~, which specifies which characters form
...
**** The =my-emacs-essentials.el= section for comments
:PROPERTIES:
:CUSTOM_ID: h:85755648-bf1f-408b-bc65-4db357fbf1dd
:END:

[[#h:7affcbca-0808-4159-8eab-faa7973aefc7][The =my-comment.el= library]] expands the built-in commenting
facilities with what makes sense to me. The ~my-comment~ command is
like the built-in ~comment-dwim~, but toggles linewise commenting
instead of appending to them by default. While the ~my-comment-timestamp-keyword~
prompts for a keyword among ~my-comment-keywords~ and formats it as
a comment with a timestamp next to it. The format of the latter is
controlled by the user option ~my-comment-timestamp-format-concise~
or ~my-comment-timestamp-format-verbose~ (the verbose is set when
...
default ([[#h:7affcbca-0808-4159-8eab-faa7973aefc7][The =my-comment.el= library]]). While I use this section of
...
**** The =my-emacs-essentials.el= section about =my-prefix.el= (prefix nested keymaps)
:PROPERTIES:
...
[[#h:f860bd85-671e-477e-8233-23a8ba43e1c6][The =my-prefix.el= library]] defines a set of keymaps with commonly
...
configurable amount of time ([[#h:ddb1070d-2f91-4224-ad43-ef03f038f787][The =my-emacs-which-key.el= module]]).
...
**** The =my-emacs-essentials.el= configuration to track recently visited files
:PROPERTIES:
...
completion ([[#h:15edf2c3-4419-4101-928a-6e224958a741][The =my-emacs-completion.el= module]]).
...
bookmarks ([[#h:22e97b4c-d88d-4deb-9ab3-f80631f9ff1d][The =my-emacs-completion.el= settings for ~consult~]]).
...
**** The =my-emacs-essentials.el= mouse configurations
:PROPERTIES:
...
**** The =my-emacs-essentials.el= settings for ~ultra-scroll~

#+begin_src emacs-lisp :tangle no
  ;; "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-essentials.el"
...
**** The =my-emacs-essentials.el= settings for ~repeat-mode~
:PROPERTIES:
...
**** The =my-emacs-essentials.el= settings for bookmarks
:PROPERTIES:
...
Also see [[#h:5685df62-4484-42ad-a062-d55ab19022e3][the =my-emacs-essentials.el= settings for registers]].
...
**** The =my-emacs-essentials.el= settings for registers
:PROPERTIES:
...
quickly ([[#h:581aa0ff-b136-4099-a321-3b86edbfbccb][The =my-emacs-essentials.el= settings for bookmarks]]). A
...
with ~savehist-mode~ ([[#h:25765797-27a5-431e-8aa4-cc890a6a913a][The =my-emacs-completion.el= settings for saving the history (~savehist-mode~)]]).
...
**** The =my-emacs-essentials.el= settings for auto revert
:PROPERTIES:
...
**** The =my-emacs-essentials.el= section for ~delete-selection-mode~
:PROPERTIES:
...
**** The =my-emacs-essentials.el= settings for tooltips
:PROPERTIES:
...
**** The =my-emacs-essentials.el= configurations for the date and time (~display-time-mode~)
:PROPERTIES:
:CUSTOM_ID: h:d4e30bcb-2f90-445f-a5ca-856e2576483a
:END:

I like to display the current date and time on the mode line([[#h:41ee38d0-9099-4444-85e4-cc8dce00fb9a][The =my-emacs-modeline.el= module]]).
...
([[#h:b26c6297-ebe8-4f99-b069-df53cfd90520][The =my-emacs-email.el= submodule for ~notmuch~ (=my-emacs-notmuch.el=)]]).
...
**** The =my-emacs-essentials.el= settings for the ~world-clock~
:PROPERTIES:
...
([[#h:b5fa481d-8549-4424-869e-91091cdf730b][The =my-emacs-window.el= module]]).
...
**** The =my-emacs-essentials.el= settings for manpages
:PROPERTIES:
:CUSTOM_ID: h:5eb16d63-ce21-4e76-8789-eef6ddbd7c00
:END:

Most buffers conform with rules we define in the ~display-buffer-alist~
([[#h:b5fa481d-8549-4424-869e-91091cdf730b][The =my-emacs-window.el= module]]). However, =M-x man= does not do
...
**** The =my-emacs-essentials.el= settings for ~proced~
:PROPERTIES:
...
**** The =my-emacs-essentials.el= arrangement to run Emacs as a server
:PROPERTIES:
...
**** The =my-emacs-essentials.el= section about ~substitute~
:PROPERTIES:
...
**** The =my-emacs-essentials.el= section about ~goto-chg~ (go to change)
:PROPERTIES:
...
**** The =my-emacs-essentials.el= section about ~expreg~ (tree-sitter mark syntactically)
:PROPERTIES:
...
binding is taken by ~my-simple-mark-sexp~ which is similar in spirit
([[#h:f15bc843-2dd1-4792-95ec-9b7b6e561804][The =my-emacs-essentials.el= section about =my-simple.el= (custom basic commands)]]).
...
**** COMMENT The =my-emacs-essentials.el= section about ~vundo~ (visualise undo steps)
:PROPERTIES:
...
command ~my/vundo-if-repeat-undo~ produces a visualisation of the
...
visualisation is a nice extra ([[#h:d3684ec0-a972-4033-a4d6-41ee16e891d9][The =init.el= macro to bind keys (~my-emacs-keybind~)]]).
...
(use-package vundo
  ...
  (defvar my/vundo-undo-functions '(undo undo-only undo-redo)
    "List of undo functions to check if we need to visualise the undo ring.")

  (defvar my/vundo-undo-command #'undo
    "Command to call if we are not going to visualise the undo ring.")

  (defun my/vundo-if-repeat-undo (&rest args)
    "Use `vundo' if the last command is among `my/vundo-undo-functions'.
In other words, start visualising the undo ring if we are going
to be cycling through the edits."
    (interactive)
    (if (and (member last-command my/vundo-undo-functions)
             (not undo-in-region))
  ...
  (mapc
   (lambda (fn)
     (advice-add fn :around #'my/vundo-if-repeat-undo))
   my/vundo-undo-functions)

  (with-eval-after-load 'pulsar
    (add-hook 'vundo-post-exit-hook #'pulsar-pulse-line-green)))
#+end_src

**** The =my-emacs-essentials.el= section about ~tmr~ (set timers)
:PROPERTIES:
...
**** The =my-emacs-essentials.el= section about ~password-store~
:PROPERTIES:
...
**** The =my-emacs-essentials.el= section about ~shell~
:PROPERTIES:
...
terminal ([[#h:14d40e5a-327e-453e-861c-3ece24351d62][The =my-emacs-essentials.el= settings for ~proced~]]).
...
because my =my-shell.el= provides a few niceties that improve the
user experience ([[#h:32f6fe0f-23c4-44cc-97cc-3e5372bd484e][The =my-shell.el= library]]).
...
the command ~my-shell~: it not only uses a unique and informative
...
My ~my-shell-mode~ defines a few extra key bindings (per the
~my-shell-mode-map~) and also implements a bookmark handler for
shell buffers ([[#h:581aa0ff-b136-4099-a321-3b86edbfbccb][The =my-emacs-essentials.el= settings for bookmarks]]).
...
**** The =my-emacs-essentials.el= section about the laptop-specific settings
:PROPERTIES:
...
**** The =my-emacs-essentials.el= settings for asynchronous code formatting
Asynchronous code formatting without cursor disruption
Apheleia is an Emacs package designed to run code formatters
asynchronously without disrupting the cursor position. Code formatters
like Shfmt, Black and Prettier ensure consistency and improve
collaboration by automating formatting, but running them on save can
introduce latency (e.g., Black takes around 200ms on an empty file)
and unpredictably move the cursor when modifying nearby text.

Apheleia solves both problems across all languages, replacing
language-specific packages like Blacken and prettier-js. It does this
by invoking formatters in an after-save-hook, ensuring changes are
applied only if the buffer remains unmodified.

To maintain cursor stability, Apheleia generates an RCS patch, applies
it selectively, and employs a dynamic programming algorithm to
reposition the cursor if necessary. If the formatting alters the
vertical position of the cursor in the window, Apheleia adjusts the
scroll position to preserve visual continuity across all displayed
instances of the buffer. This allows enjoying automated code
formatting without sacrificing editor responsiveness or usability.
...
**** The =my-emacs-essentials.el= section for better ~help~ buffer
...
**** The =my-emacs-essentials.el= block for ~ibuffer~
   ...
   #+BEGIN_SRC emacs-lisp "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-essentials.el"
...
**** The =my-emacs-essentials.el= call to ~provide~
:PROPERTIES:
...
*** The =my-emacs-modeline.el= module
:PROPERTIES:
...
([[#h:d4e30bcb-2f90-445f-a5ca-856e2576483a][The =my-emacs-essentials.el= configurations for the date and time (~display-time-mode~)]])
or the ~notmuch-indicator~ ([[#h:0f12f144-ca9f-4d07-8563-c33b3521bfb5][The =my-emacs-notmuch.el= section about the ~notmuch-indicator~]]).

Most items that go into the ~mode-line-format~ are defined in
[[#h:935adc09-abaa-4413-a5ab-a7a562081c20][the =my-modeline.el= library]].
...
~spacious-padding~ package ([[#h:6c118185-fcb1-4c9a-93af-71814cb84279][The =my-emacs-theme.el= section for ~spacious-padding~]]).
It adds, among others, a box effect to mode line constructs.
To make the faces of ~my-modeline.el~ look right in this scenario, I
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-modeline.el" :mkdirp yes
;;; Mode line
...
**** The =my-emacs-modeline.el= section about ~keycast~
:PROPERTIES:
...
with my customised mode line ([[#h:41ee38d0-9099-4444-85e4-cc8dce00fb9a][The =my-emacs-modeline.el= module]]).

#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-modeline.el"
;;; Keycast mode
(use-package keycast
  :ensure t
  :after my-modeline
...
*** The =my-emacs-completion.el= module
:PROPERTIES:
...
**** The =my-emacs-completion.el= settings for completion styles
:PROPERTIES:
...
- orderless :: This is the only completion style I use which is not
  ...
  ([[#h:7cc77fd0-8f98-4fc0-80be-48a758fcb6e2][The =my-emacs-completion.el= for the ~orderless~ completion style]]).
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-completion.el" :mkdirp yes
...
**** The =my-emacs-completion.el= for the ~orderless~ completion style
:PROPERTIES:
:CUSTOM_ID: h:7cc77fd0-8f98-4fc0-80be-48a758fcb6e2
:END:

The ~orderless~ package by Omar Antolín Camarena provides one of the
completion styles that I use ([[#h:14b09958-279e-4069-81e3-5a16c9b69892][The =my-emacs-completion.el= settings for completion styles]]).
...
to use a specific pattern for that component. My =my-orderless.el=
defines such style dispatchers as postfix operators: they are added to
the end of the input ([[#h:114c6e8c-697a-43e9-8fd9-52872997e1c9][The =my-orderless.el= library]]). The equals
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-completion.el"
...
**** The =my-emacs-completion.el= settings to ignore letter casing
:PROPERTIES:
...
**** The =my-emacs-completion.el= settings for recursive minibuffers
:PROPERTIES:
...
**** The =my-emacs-completion.el= settings for default values
:PROPERTIES:
...
**** The =my-emacs-completion.el= settings for common interactions
:PROPERTIES:
...
- Finally the ~file-name-shadow-mode~ is a neat little feature to
  ...
  ([[#h:14b09958-279e-4069-81e3-5a16c9b69892][The =my-emacs-completion.el= settings for completion styles]]).
...
**** The =my-emacs-completion.el= generic minibuffer UI settings
:PROPERTIES:
:CUSTOM_ID: h:de61a607-0bdf-462b-94cd-c0898319590e
:END:

These are some settings for the default completion user interface.
These do not come into effect unless ~my-emacs-completion-ui~ is set
...
- [[#h:cff33514-d3ac-4c16-a889-ea39d7346dc5][The =my-emacs-completion.el= submodule for ~vertico~ (=my-emacs-vertico.el=)]]
- [[#h:e9fb09f0-f2e6-4943-83bf-e5fadcfa530a][The =my-emacs-completion.el= submodule for ~mct~ (=my-emacs-mct.el=)]]
...
**** The =my-emacs-completion.el= settings for saving the history (~savehist-mode~)
:PROPERTIES:
...
- [[#h:804b858f-7913-47ef-aaf4-8eef5b59ecb4][The =my-emacs-completion.el= for in-buffer completion popup and preview (~corfu~)]]
- [[#h:5685df62-4484-42ad-a062-d55ab19022e3][The =my-emacs-essentials.el= settings for registers]]
- [[#h:32f6fe0f-23c4-44cc-97cc-3e5372bd484e][The =my-shell.el= library]]
...
**** The =my-emacs-completion.el= settings for dynamic text expansion (~dabbrev~)
:PROPERTIES:
...
static, user-defined abbreviations ([[#h:fd84b79a-351e-40f0-b383-bf520d77834b][The =my-emacs-completion.el= settings for static text expansion (~abbrev~)]]).
...
**** The =my-emacs-completion.el= settings for static text expansion (~abbrev~)
:PROPERTIES:
...
to what we have specified ([[#h:567bb00f-1d82-4746-93e5-e0f60721728a][The =my-emacs-completion.el= settings for dynamic text expansion (~dabbrev~)]]).
...
As you will notice below, I use the ~my-emacs-abbrev~ macro to make
it easier to define these ([[#h:e7a12825-7848-42bd-b99b-b87903012814][The =init.el= macro to define abbreviations (~my-emacs-abbrev~)]]).
...
**** The =my-emacs-completion.el= for in-buffer completion popup (~corfu~ and ~cape~)
:PROPERTIES:
...
Also see [[#h:567bb00f-1d82-4746-93e5-e0f60721728a][the =my-emacs-completion.el= settings for dynamic text expansion (~dabbrev~)]].
...
**** The =my-emacs-completion.el= settings for ~consult~
:PROPERTIES:
...
- A case where filtering is in use is the ~consult-buffer~ command,
  ...
  bookmarks ([[#h:581aa0ff-b136-4099-a321-3b86edbfbccb][The =my-emacs-essentials.el= settings for bookmarks]]),
...
- Asynchronous input pertains to the intersection between Emacs and
  ...
  by wrapping the former inside of =#=. So the input =#my-#completion=
  will send =my-= to the ~grep~ program and then use =completion=
  inside of the minibuffer to perform the subsequent pattern-matching
  (e.g. with help from ~orderless~ ([[#h:7cc77fd0-8f98-4fc0-80be-48a758fcb6e2][The =my-emacs-completion.el= for the ~orderless~ completion style]]).
...
Also check: [[#h:e0f9c30e-3a98-4479-b709-7008277749e4][The =my-emacs-search.el= module]].
...
**** The =my-emacs-completion.el= section about ~embark~
:PROPERTIES:
...
do with all well-behaved buffers ([[#h:50f8b1e4-b14e-453f-a37e-1c0e495ab80f][The =my-emacs-window.el= rules for displaying buffers (~display-buffer-alist~)]]).
...
Another downside, which is also true for ~which-key~ ([[#h:ddb1070d-2f91-4224-ad43-ef03f038f787][The =my-emacs-which-key.el= module]]),
...
Up until 2025-04-10, I maintained a =my-embark.el= package as part
...
**** The =my-emacs-completion.el= section to configure completion annotations (~marginalia~)
:PROPERTIES:
...
accordingly ([[#h:14b09958-279e-4069-81e3-5a16c9b69892][The =my-emacs-completion.el= settings for completion styles]]).
...
**** The =my-emacs-completion.el= setting to load a minibuffer UI submodule
:PROPERTIES:
...
***** The =my-emacs-completion.el= submodule for ~mct~ (=my-emacs-mct.el=)
:PROPERTIES:
:CUSTOM_ID: h:e9fb09f0-f2e6-4943-83bf-e5fadcfa530a
:END:

[ This is subject to a user option: [[#h:db620e44-fc8f-45b8-a7fd-ae1d0b7ce86c][The =my-emacs-completion.el= setting to load a minibuffer UI submodule]]. ]
...
pop up the minibuffer eagerly ([[#h:cff33514-d3ac-4c16-a889-ea39d7346dc5][The =my-emacs-completion.el= submodule for ~vertico~ (=my-emacs-vertico.el=)]]).

#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-mct.el" :mkdirp yes
...
***** The =my-emacs-completion.el= submodule for ~vertico~ (=my-emacs-vertico.el=)
:PROPERTIES:
...
The extras I provide for ~vertico~ are in [[#h:3796a4c9-8659-4782-8aaa-3cf4e950927d][the =my-vertico.el= library]].

#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-vertico.el" :mkdirp yes
...
- =M-s o= (~occur~) :: Search for the given regular expression
  ...
  extensions: [[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =my-emacs-search.el= extras provided by the =my-search.el= library]].
...
**** The =my-emacs-search.el= on isearch lax space
:PROPERTIES:
...
([[#h:e0f9c30e-3a98-4479-b709-7008277749e4][The =my-emacs-search.el= module]]).
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-search.el" :mkdirp yes
...
**** The =my-emacs-search.el= settings for isearch highlighting
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-search.el"
(use-package isearch
  :ensure nil
  :demand t
  :config
...
**** The =my-emacs-search.el= on isearch match counter
:PROPERTIES:
...
**** The =my-emacs-search.el= tweaks to the isearch motion behaviour
:PROPERTIES:
...
the opposite end with =C-RET= while in an isearch (~my-search-isearch-other-end~).

[ See: [[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =my-emacs-search.el= extras provided by the =my-search.el= library]]. ]
...
**** The =my-emacs-search.el= tweaks for the occur buffer
:PROPERTIES:
:CUSTOM_ID: h:85aca4da-b89b-4fbe-89e9-3ec536ad7b0d
:END:

Here I am making some minor tweaks to =*occur*= buffer (remember to
read the introduction to this section ([[#h:e0f9c30e-3a98-4479-b709-7008277749e4][The =my-emacs-search.el= module]])).
...
The latter depends on my =my-common.el= library, which is loaded
early at startup ([[#h:713ede33-3802-40c6-a8e3-7e1fc0d0a924][The =my-emacs-essentials.el= block that loads my custom libraries]]).
...
**** The =my-emacs-search.el= modified isearch and occur key bindings
:PROPERTIES:
...
**** The =my-emacs-search.el= extras provided by the =my-search.el= library
:PROPERTIES:
:CUSTOM_ID: h:b902e6a3-cdd2-420f-bc99-3d973c37cd20
:END:

My =my-search.el= provides lots of useful extras that I use on a
regular basis ([[#h:0cab525d-fcbc-478c-b914-bbd69e33c635][The =my-search.el= library]]).
...
- Place the cursor on the opposite end of an isearch when exiting. Do
  it with =C-RET= while in isearch ([[#h:e6fd4ff0-86c7-423b-9cf3-0a9061505136][The =my-emacs-search.el= tweaks to the isearch motion behaviour]]).

- Delete the non-matching portion of a query in isearch with a single
  backspace instead of doing it character-by-character.

- Type =M-s M-<= (~my-search-isearch-beginning-of-buffer~) or =M-s
  M->= (~my-search-isearch-end-of-buffer~) to search for the symbol
...
where windows/buffers are displayed ([[#h:50f8b1e4-b14e-453f-a37e-1c0e495ab80f][The =my-emacs-window.el= rules for displaying buffers (~display-buffer-alist~)]]):

- Type =M-s s= (~my-search-outline~) to use minibuffer completion
  ...
  minibuffer user interface ([[#h:15edf2c3-4419-4101-928a-6e224958a741][The =my-emacs-completion.el= module]]).
  ...
  Consult command on =M-s M-s= ([[#h:22e97b4c-d88d-4deb-9ab3-f80631f9ff1d][The =my-emacs-completion.el= settings for ~consult~]]).

- Type =M-s M-o= (~my-search-occur-outline~) to produce an outline
  of the given buffer. What constitutes an outline is defined in the
  user option ~my-search-outline-regexp-alist~. I only configure it
  for the major modes I care about, though the mechanism should work
  for every buffer that has an outline.

- Type =M-s M-t= (~my-search-grep-todo-keywords~) to produce an
  =*occur*= buffer that matches keywords like =TODO=. The full regular
  expression is specified in the user option ~my-search-todo-keywords~.

- Type =C-u M-s M-t= or =M-s M-T= (~my-search-git-grep-todo-keywords~)
  ...
  ([[#h:9a3581df-ab18-4266-815e-2edd7f7e4852][The =my-emacs-search.el= setup for editable grep buffers (~wgrep~)]]).

- Type =M-s g= (~my-search-grep~) to perform a "local grep" across
...
matching ([[#h:22e97b4c-d88d-4deb-9ab3-f80631f9ff1d][The =my-emacs-completion.el= settings for ~consult~]]).
...
**** The =my-emacs-search.el= tweaks to ~xref~, ~re-builder~ and ~grep~
:PROPERTIES:
...
**** The =my-emacs-search.el= setup for editable grep buffers (~grep-edit-mode~ or ~wgrep~)
:PROPERTIES:
...
([[#h:1b53bc10-8b1b-4f68-bbec-165909761e43][The =my-emacs-dired.el= section about ~wdired~ (writable Dired)]]).
I use this in tandem with my ~my-search-grep~ command.
...
**** The =my-emacs-search.el= call to ~provide~
:PROPERTIES:
:CUSTOM_ID: h:c8b2f021-fe5a-4f6b-944c-20340f764fb2
:END:
...
**** The =my-emacs-dired.el= settings for common operations
:PROPERTIES:
...
([[#h:2e005bd1-d098-426d-91f9-2a31a6e55caa][The =my-emacs-dired.el= section about =trashed.el=]]).

#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-dired.el" :mkdirp yes
...
**** The =my-emacs-dired.el= switches for ~ls~ (how files are listed)
:PROPERTIES:
:CUSTOM_ID: h:679e4460-b306-450f-aa20-497243057e02
:END:

As I already explained, Dired is a layer of interactivity on top of
standard Unix tools ([[#h:f8b08a77-f3a8-42fa-b1a9-f940348889c3][The =my-emacs-dired.el= module]]). We can see
...
- =-h= :: Make file sizes easier to read, such as =555k= instead of
  =568024= (the size of =my-emacs.org= as of this writing).
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-dired.el"
(use-package dired
  :ensure nil
  :commands (dired)
  :config
...
**** The =my-emacs-dired.el= setting for dual-pane Dired
:PROPERTIES:
...
history ([[#h:25765797-27a5-431e-8aa4-cc890a6a913a][The =my-emacs-completion.el= settings for saving the history (~savehist-mode~)]]).
...
**** The =my-emacs-dired.el= settings to open files externally
:PROPERTIES:
...
**** The =my-emacs-dired.el= miscellaneous tweaks
:PROPERTIES:
...
([[#h:679e4460-b306-450f-aa20-497243057e02][The =my-emacs-dired.el= switches for ~ls~ (how files are listed)]]).
...
([[#h:fbe6f9da-25ee-46a3-bb03-8fa7c1d48dab][The =my-emacs-essentials.el= settings for ~repeat-mode~]]).
...
**** The =my-emacs-dired.el= section about various conveniences
:PROPERTIES:
...
- the key binding for ~dired-do-open~, which opens the file or
  directory externally ([[#h:d7d5e619-3d50-4d9e-a951-7262462a60c9][The =my-emacs-dired.el= settings to open files externally]]).
...
**** The =my-emacs-dired.el= section about my extras (=my-dired.el=)
:PROPERTIES:
...
actions ([[#h:92cfefcf-8c20-43cd-b47c-ffe0c5fe1887][The =my-dired.el= library]]). Chief among them is the
~my-dired-limit-regexp~ (bound to =C-c C-l=), which is an easier way
...
All this is fine, but with ~my-dired-limit-regexp~ I simply provide
...
current directory. I do this with ~my-dired-search-flat-list~.

The other commands have situational uses. For example, the
~my-dired-grep-marked-files~ is something I have only used a few
times where ~my-search-grep~ would produce too many results in a
given directory ([[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =my-emacs-search.el= extras provided by the =my-search.el= library]]).
...
**** The =my-emacs-dired.el= section about ~dired-subtree~
:PROPERTIES:
...
**** The =my-emacs-dired.el= section about ~wdired~ (writable Dired)
:PROPERTIES:
:CUSTOM_ID: h:1b53bc10-8b1b-4f68-bbec-165909761e43
:END:

As noted in the introduction, Dired can be made writable
([[#h:f8b08a77-f3a8-42fa-b1a9-f940348889c3][The =my-emacs-dired.el= module]]). This way, we can quickly rename
...
**** The =my-emacs-dired.el= section about ~image-dired~
:PROPERTIES:
...
**** The =my-emacs-dired.el= section about ~dired-preview~
:PROPERTIES:
...
**** The =my-emacs-dired.el= section about multimedia previews (~ready-player~)
:PROPERTIES:
...
packages ([[#h:c345ff72-d905-482a-a885-fc111252aacf][The =my-emacs-dired.el= section about ~dired-preview~]]).
...
**** The =my-emacs-dired.el= section about =trashed.el=
:PROPERTIES:
...
*** The =my-emacs-dired.el= section about ~mandoura~ (~mpv~ media player)
:PROPERTIES:
...
this is a feature built into Emacs 30, though it alsoi s available as
...
*** The =my-emacs-dired.el= call to ~provide~
:PROPERTIES:
:CUSTOM_ID: h:c8b2f021-fe5a-4f6b-944c-20340f764fb2
:END:
...
**** The =my-emacs-window.el= section about running commands in popup frames
:PROPERTIES:
...
Emacs daemon process or ~server-mode~ ([[#h:7709b7e9-844f-49f3-badf-784aacec4bca][The =my-emacs-essentials.el= arrangement to run Emacs as a server]]).
We can thus call ~emacsclient~ from outside Emacs and instruct it to
run any arbitrary Emacs Lisp code.

My =my-window.el= provides the ~my-window-define-with-popup-frame~
macro which streamlines the creation of commands that have the desired
behaviour ([[#h:35b8a0a5-c447-4301-a404-bc274596238d][The =my-window.el= library]]). All we need is to call the
...
(with-eval-after-load 'tmr
  (add-hook 'tmr-timer-created-functions #'my-window-delete-popup-frame))

(with-eval-after-load 'my-project
  (add-hook 'my-project-switch-hook #'my-window-delete-popup-frame))
#+end_example

The commands I configure are for ~org-capture~ and ~tmr~ as well as my
custom ~my-project-switch~:

- [[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =my-emacs-org.el= Org capture templates (~org-capture~)]]
- [[#h:6dce278e-86c7-4620-9271-d5af5499f0cc][The =my-emacs-essentials.el= section about ~tmr~ (set timers)]]
- [[#h:7dcbcadf-8af6-487d-b864-e4ce56d69530][The =my-emacs-git.el= section about =project.el=]]
...
# Same idea for the `my-project-switch` command.
emacsclient -e '(my-window-popup-my-project-switch)'
#+end_example

**** The =my-emacs-window.el= section about uniquifying buffer names
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-window.el" :mkdirp "yes"
...
**** The =my-emacs-window.el= section about line highlighting (~hl-line-mode~)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-window.el"
;;;; Line highlight
(use-package hl-line
  :ensure nil
...
**** The =my-emacs-window.el= section about negative space highlighting (~whitespace-mode~)
:PROPERTIES:
:CUSTOM_ID: h:ad2945c5-992d-4c82-a8b8-652f47ebdbea
:END:

Much like ~hl-line-mode~, I normally do not use the ~whitespace-mode~
([[#h:ec156866-3d25-42e7-8247-b190103b6f17][The =my-emacs-window.el= section about line highlighting (~hl-line-mode~)]]).
...
**** The =my-emacs-window.el= section about line numbers (~display-line-numbers-mode~)
:PROPERTIES:
...
**** The =my-emacs-window.el= rules for displaying buffers (~display-buffer-alist~)
:PROPERTIES:
...
In my =my-window.el= library, I define functions that determine how
a buffer should be displayed, given size considerations ([[#h:35b8a0a5-c447-4301-a404-bc274596238d][The =my-window.el= library]]).
You will find the functions ~my-window-shell-or-term-p~ to determine
what a shell or terminal is, ~my-window-display-buffer-below-or-pop~
to display the buffer below the current one or to its side depending
on how much width is available, and ~my-window-select-fit-size~ to
...
height or width. These are relevant for ~my-window-display-buffer-below-or-pop~
...
**** The =my-emacs-window.el= setting to enable ~visual-line-mode~ in some contexts
:PROPERTIES:
...
**** The =my-emacs-window.el= settings to truncate some buffers silently
:PROPERTIES:
:CUSTOM_ID: h:b6754236-3d08-4448-9c86-3e5923737e4b
:END:

This here is the opposite of what we saw above ([[#h:bf3f50d5-c5db-4704-9498-9e8bfe1b45a8][The =my-emacs-window.el= setting to enable ~visual-line-mode~ in some contexts]]).
...
truncation without displaying a message about the fact ([[#h:3fccfadf-22e9-457f-b9fd-ed1b48600d23][The =my-common.el= library]]).
...
**** The =my-emacs-window.el= section key bindings
:PROPERTIES:
...
**** The =my-emacs-window.el= section about ~beframe~
:PROPERTIES:
:CUSTOM_ID: h:77e4f174-0c86-460d-8a54-47545f922ae9
:END:

[ Also see: [[#h:7dcbcadf-8af6-487d-b864-e4ce56d69530][The =my-emacs-git.el= section about =project.el=]]. ]
...
**** The =my-emacs-window.el= configuration of ~undelete-frame-mode~ and ~winner-mode~
:PROPERTIES:
:CUSTOM_ID: h:2df15080-77f9-45f8-a3b5-1adddc70a512
:END:

Since I am using my ~beframe~ package to isolate buffers per frame
([[#h:77e4f174-0c86-460d-8a54-47545f922ae9][The =my-emacs-window.el= section about ~beframe~]]), I appreciate the
...
**** The =my-emacs-window.el= keys for window motions (~windmove~)
:PROPERTIES:
...
**** The =my-emacs-window.el= use of contextual header line (~breadcrumb~)
:PROPERTIES:
...
**** The =my-emacs-window.el= golden ratio windows (~golden-ratio~)
Window space is not large enough for this screen so this package hides
makes secondary windows smaller.
...
**** The =my-emacs-window.el= ~provide~ form
:PROPERTIES:
...
**** The =my-emacs-git.el= section about ediff
:PROPERTIES:
...
integrates with ~magit~ ([[#h:b08af527-9ebf-4425-ac3a-24b4f371a4fd][The =my-emacs-git.el= section about ~magit~ (great Git client)]]).

#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-git.el" :mkdirp yes
;;;; `ediff'
(use-package ediff
  :ensure nil
...
**** The =my-emacs-git.el= section about =project.el=
:PROPERTIES:
...
built-in =vc.el= ([[#h:50add1d8-f0f4-49be-9e57-ab280a4aa300][The =my-emacs-git.el= section about =vc.el= and related]]).
...
limited to that frame ([[#h:77e4f174-0c86-460d-8a54-47545f922ae9][The =my-emacs-window.el= section about ~beframe~]]).

Note that [[#h:654cfe49-f59d-4a0d-84ee-094ddce06848][the =my-project.el= library]] defines functionality that is
useful, but which I personally do not need. I am thus not including it
here, but do take a look.

#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-git.el"
;;;; `project'
(use-package project
  :ensure nil
  :bind
...
**** The =my-emacs-git.el= section about ~diff-mode~
:PROPERTIES:
...
example, fold the invidual diff hunks and move between them ([[#h:ffff5f7b-a62b-4d4a-ae29-af75402e5c35][The =my-emacs-langs.el= settings for ~outline-minor-mode~]]).
Personally, I combine this feature with my ~my-search-outline~
command to quickly jump to an outline heading using minibuffer
completion ([[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =my-emacs-search.el= extras provided by the =my-search.el= library]]).
...
**** The =my-emacs-git.el= section about =vc.el= and related
:PROPERTIES:
...
([[#h:8b426a69-e3cd-42ac-8788-f41f6629f879][The =my-emacs-git.el= section about ~diff-mode~]]).
...
([[#h:b08af527-9ebf-4425-ac3a-24b4f371a4fd][The =my-emacs-git.el= section about ~magit~ (great Git client)]]).
...
([[#h:257b9118-4407-410e-a56a-134fd5d1a3b4][The =my-emacs-git.el= section about ~agitate~]]).
...
**** The =my-emacs-git.el= section about ~agitate~
:PROPERTIES:
:CUSTOM_ID: h:257b9118-4407-410e-a56a-134fd5d1a3b4
:END:

This is an experimental package of mine whose role is to extend the
functionality provided by the =vc= suite of tools ([[#h:50add1d8-f0f4-49be-9e57-ab280a4aa300][The =my-emacs-git.el= section about =vc.el= and related]]),
as well as that of ~diff-mode~ ([[#h:8b426a69-e3cd-42ac-8788-f41f6629f879][The =my-emacs-git.el= section about ~diff-mode~]]).
...
**** The =my-emacs-git.el= section about ~magit~ (great Git interface)
:PROPERTIES:
...
all we need to select targets with ease ([[#h:15edf2c3-4419-4101-928a-6e224958a741][The =my-emacs-completion.el= module]]).
...
([[#h:50add1d8-f0f4-49be-9e57-ab280a4aa300][The =my-emacs-git.el= section about =vc.el= and related]]).
...
- Link to files regardless of file type. This includes special links
  ...
  locally and integrated with Emacs ([[#h:755e195b-9471-48c7-963b-33055969b4e2][The =my-emacs-email.el= module]]).
...
**** The =my-emacs-org.el= section on the ~calendar~
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-org.el" :mkdirp yes
;;; Calendar
...
**** The =my-emacs-org.el= section about appointment reminders (=appt.el=)
:PROPERTIES:
...
([[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =my-emacs-org.el= Org agenda settings]]). I deepen this
...
done at a specific time, such as coaching sessions ([[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =my-emacs-org.el= Org capture templates (~org-capture~)]]).

#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-org.el"
...
**** The =my-emacs-org.el= section with basic Org settings
:PROPERTIES:
...
**** The =my-emacs-org.el= Org indent mode settings
:PROPERTIES:
...
**** The =my-emacs-org.el= Org to-do and refile settings
:PROPERTIES:
...
which I consider helpful ([[#h:0884658e-9eb5-47e3-9338-66e09004a1a0][The =my-emacs-org.el= Org time/state logging]]).
...
date ([[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =my-emacs-org.el= Org agenda settings]]).
...
the capture phase ([[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =my-emacs-org.el= Org capture templates (~org-capture~)]]).
...
**** The =my-emacs-org.el= Org heading tags
:PROPERTIES:
...
- [[#h:024dd541-0061-4a10-b10b-b17dcd4794b9][The =my-emacs-org.el= Org to-do and refile settings]]
- [[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =my-emacs-org.el= Org agenda settings]]
...
**** The =my-emacs-org.el= Org time/state logging
:PROPERTIES:
...
appointment ([[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =my-emacs-org.el= Org agenda settings]]). This data
...
**** The =my-emacs-org.el= Org link settings
:PROPERTIES:
...
- [[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =my-emacs-org.el= Org capture templates (~org-capture~)]]
- [[#h:49890997-448e-408d-bebe-2003259bb125][The =my-emacs-notmuch.el= glue code for ~org-capture~ (=ol-notmuch.el=)]]
...
[[#h:e03df1e0-b43e-49b5-978e-6a511165617c][The =my-emacs-org.el= section with basic Org settings]].
...
**** The =my-emacs-org.el= Org code block settings
:PROPERTIES:
...
- =#+BEGIN_EXPORT= :: Evaluate the code for the given export target
  ...
  or keeping both of them ([[#h:bd11d4d8-6e9f-4536-87a4-4018783bf8f5][The =my-emacs-org.el= Org export settings]]).
...
**** The =my-emacs-org.el= Org export settings
:PROPERTIES:
...
I showed for Org Babel ([[#h:1f5a0d46-5202-48dd-8048-b48ce17f3df8][The =my-emacs-org.el= Org code block settings]]).
...
**** The =my-emacs-org.el= Org capture templates (~org-capture~)
:PROPERTIES:
...
with the general purpose command ~my-search-outline~
([[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =my-emacs-search.el= extras provided by the =my-search.el= library]]).
...
- When I want to capture data that I am not yet sure about, I add it
  ...
  heading in the same file using the ~org-refile~ command ([[#h:024dd541-0061-4a10-b10b-b17dcd4794b9][The =my-emacs-org.el= Org to-do and refile settings]]).
...
- Tasks that have an inherent time component are given a =SCHEDULED=
  ...
  tasks I want to see on my daily agenda ([[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =my-emacs-org.el= Org agenda settings]]).
...
**** The =my-emacs-org.el= Org agenda settings
:PROPERTIES:
...
task in the usual place ([[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =my-emacs-org.el= Org capture templates (~org-capture~)]]).
...
this order ([[#h:9974eac8-2167-45c4-90e0-12dd877403da][The =my-org.el= library]]).:
...
**** The =my-emacs-org.el= paste link with description automatically
:PROPERTIES:
:ID:       084693d7-a00f-471b-bfa2-eda9da1b4718
:END:
...
**** The =my-emacs-org.el= call to ~provide~
:PROPERTIES:
...
**** The =my-emacs-langs.el= settings for TAB
:PROPERTIES:
...
completion at point ([[#h:804b858f-7913-47ef-aaf4-8eef5b59ecb4][The =my-emacs-completion.el= for in-buffer completion popup (~corfu~)]]).
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-langs.el" :mkdirp yes
...
**** The =my-emacs-langs.el= settings for "electric" behaviour
:PROPERTIES:
...
of characters, I use my ~my-pair-insert~ command ([[#h:48d2bab5-241f-49dd-9554-8a4dcedce75e][The =my-emacs-essentials.el= section about =my-pair.el= (insert character pairs)]]).

#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-langs.el"
...
**** The =my-emacs-langs.el= settings ~show-paren-mode~
:PROPERTIES:
...
**** The =my-emacs-langs.el= settings for plain text (no double spaces, ~auto-fill-mode~)
:PROPERTIES:
...
which corresponds to the ~my-simple-unfill-region-or-paragraph~
command ([[#h:f15bc843-2dd1-4792-95ec-9b7b6e561804][The =my-emacs-essentials.el= section about =my-simple.el= (custom basic commands)]]).
...
**** The =my-emacs-langs.el= settings for common file types
:PROPERTIES:
:CUSTOM_ID: h:1a132701-9487-4af0-8759-a8d64d02bf1f
:END:

As I explained above about ~auto-mode-alist~ ([[#h:c5fbd6dd-2982-4fca-aeed-d0e750d399bd][The =my-emacs-langs.el= settings for plain text]]),
...
**** The =my-emacs-langs.el= settings for ~eldoc~
:PROPERTIES:
...
**** The =my-emacs-langs.el= settings for ~eglot~ (LSP client)
:PROPERTIES:
...
- Code completion :: This can be visualised for in-buffer
  ...
  ([[#h:804b858f-7913-47ef-aaf4-8eef5b59ecb4][The =my-emacs-completion.el= for in-buffer completion popup (~corfu~)]]).
...
- Code linting :: To display suggestions, warnings, or errors. These
  ...
  ([[#h:df6d1b52-0306-4ace-9099-17dded11fbed][The =my-emacs-langs.el= settings for code linting (~flymake~)]]).
...
- Code navigation and cross-referencing :: While over a symbol, use a
  ...
  [ Features such as the definition of the outline should, in
    ...
    folding capabilties for outline headings ([[#h:ffff5f7b-a62b-4d4a-ae29-af75402e5c35][The =my-emacs-langs.el= settings for ~outline-minor-mode~]]).
    I usually navigate the outline using minibuffer completion, with
    the help of my ~my-search-outline~ command ([[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =my-emacs-search.el= extras provided by the =my-search.el= library]]). ]
...
**** The =my-emacs-langs.el= settings for very long lines
:PROPERTIES:
...
**** The =my-emacs-langs.el= settings for ~markdown-mode~
:PROPERTIES:
...
**** The =my-emacs-langs.el= settings for ~csv-mode~
:PROPERTIES:
...
**** The =my-emacs-langs.el= settings for ~sxhkdrc-mode~
:PROPERTIES:
...
**** The =my-emacs-langs.el= settings for spell checking
:PROPERTIES:
:CUSTOM_ID: h:115806c4-88b0-43c1-8db2-d9d8d20a5c17
:END:

For spell checking on-demand, I rely on the built-in ~flyspell~
feature that I complement with some small extras ([[#h:551718b7-fb2d-4a4a-804e-3118001b96ec][The =my-spell.el= library]]).
...
**** The =my-emacs-langs.el= settings for code linting (~flymake~)
:PROPERTIES:
...
LSP linter output ([[#h:92258aa8-0d8c-4c12-91b4-5f44420435ce][The =my-emacs-langs.el= settings for ~eglot~]]).
...
**** The =my-emacs-langs.el= settings for ~outline-minor-mode~
:PROPERTIES:
...
~outline-minor-mode~, courtesy of the command ~my-search-outline~
([[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =my-emacs-search.el= extras provided by the =my-search.el= library]]).
...
**** The =my-emacs-langs.el= settings for ~dictionary~
:PROPERTIES:
...
**** The =my-emacs-langs.el= settings for ~altcaps~ (alternating letter casing)
:PROPERTIES:
...
**** The =my-emacs-langs.el= settings for ~denote~ (notes and file-naming)
:PROPERTIES:
...
***** The =my-emacs-langs.el= integration between Consult and Denote (~consult-denote~)
:PROPERTIES:
:CUSTOM_ID: h:ee82e629-fb05-4c75-9175-48a760a25691
:END:

This is another package of mine which extends my ~denote~ package
([[#h:e86a66dc-7ef9-4f09-ad7e-946de2034e8d][The =my-emacs-langs.el= settings for ~denote~ (notes and file-naming)]]).

This is glue code to integrate ~denote~ with Daniel Mendler's
~consult~ ([[#h:22e97b4c-d88d-4deb-9ab3-f80631f9ff1d][The =my-emacs-completion.el= settings for ~consult~]]). The
...
***** The =my-emacs-langs.el= extension for Denote Org extras (~denote-org~)
:PROPERTIES:
...
***** The =my-emacs-langs.el= extension for Denote sequence notes or folgezettel (~denote-sequence~)
:PROPERTIES:
...
***** The =my-emacs-langs.el= extension for Denote Markdown extras (~denote-markdown~)
:PROPERTIES:
...
***** The =my-emacs-langs.el= extension for Denote Journal extras (~denote-journal~)
:PROPERTIES:
...
journal entry for the current date or create a new entry ([[#h:94d48381-1711-4d6b-8449-918bc1e3836c][The =my-emacs-org.el= section on the ~calendar~]]).
...
***** The =my-emacs-langs.el= extension for Denote Silo extras (~denote-silo~)
:PROPERTIES:
...
**** The =my-emacs-langs.el= settings for ~logos~ (writing extras and buffer navigation)
:PROPERTIES:
...
agenda and has a link back to the original email ([[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =my-emacs-org.el= Org capture templates]]).
...
**** The =my-emacs-email.el= basic settings (including =authinfo=)
:PROPERTIES:
...
or logging in to IRC with the ~rcirc~ command ([[#h:f23d9cef-ab7b-4486-a070-9c7ae664eadf][The =my-emacs-web.el= module]]).
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-email.el" :mkdirp yes
...
**** The =my-emacs-email.el= message composition and encryption settings (=message.el=)
:PROPERTIES:
...
attach them from there ([[#h:deccd4f1-c1bd-4b02-993e-914f57ea0fac][The =my-emacs-email.el= integration with Dired for email attachments (~gnus-dired-mode~)]])
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-email.el"
...
**** The =my-emacs-email.el= integration with Dired for email attachments (~gnus-dired-mode~)
:PROPERTIES:
...
composition buffer ([[#h:f8b08a77-f3a8-42fa-b1a9-f940348889c3][The =my-emacs-dired.el= module]]). Do it by typing
...
([[#h:49815c07-737f-4f10-95ee-63640f9ae18b][The =my-emacs-email.el= message composition and encryption settings (=message.el=)]]).
...
**** The =my-emacs-email.el= settings for ~sendmail~ (SMTP)
:PROPERTIES:
...
**** The =my-emacs-email.el= loading of the email client and call to ~provide~
:PROPERTIES:
...
enough ([[#h:b26c6297-ebe8-4f99-b069-df53cfd90520][The =my-emacs-email.el= submodule for ~notmuch~ (=my-emacs-notmuch.el=)]]).
...
**** The =my-emacs-email.el= submodule for ~notmuch~ (=my-emacs-notmuch.el=)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-notmuch.el" :mkdirp yes
...
***** The =my-emacs-notmuch.el= section about the account settings
:PROPERTIES:
:CUSTOM_ID: h:2fca749c-a84e-415c-9ac0-b1518d06fa51
:END:

To actually use the ~notmuch~ mail user agent ([[#h:b26c6297-ebe8-4f99-b069-df53cfd90520][The =my-emacs-email.el= submodule for ~notmuch~ (=my-emacs-notmuch.el=)]]),
...
Notice that I use the ~my-common-auth-get-field~ function, which
reads data from my encrypted =~/.authinfo.gpg= file ([[#h:24f17614-ee3d-4cd1-bbe3-f53e4f39c281][The =my-emacs-email.el= basic settings (including =authinfo=)]]).
In =my-common.el=, I define a few helper functions that are general
in nature and may be used by other Elisp snippets of mine ([[#h:3fccfadf-22e9-457f-b9fd-ed1b48600d23][The =my-common.el= library]]).
...
#+begin_example emacs-lisp
(setq notmuch-identities '("Skoryk Yurii <someone@somewhere.com>" "Yurii <just-my-nickname@somewhere.com>"))
#+end_example
...
***** The =my-emacs-notmuch.el= section about the general user interface
:PROPERTIES:
:CUSTOM_ID: h:2c9a555f-d3ec-4d0e-bad6-6c26b576a49a
:END:

When we use the ~notmuch~ Emacs command to get into the mail user
agent ([[#h:b26c6297-ebe8-4f99-b069-df53cfd90520][The =my-emacs-email.el= submodule for ~notmuch~ (=my-emacs-notmuch.el=)]]),
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-notmuch.el"
;;;; General UI
(use-package notmuch
  :defer t
  :config
...
***** The =my-emacs-notmuch.el= section about the presentation of search buffers
:PROPERTIES:
...
***** The =my-emacs-notmuch.el= section about tag settings
:PROPERTIES:
...
=my-notmuch.el= ([[#h:dbd2b775-8f51-451a-8e24-9c8424fd2e89][The =my-emacs-notmuch.el= custom extensions (per =my-notmuch.el=)]]).
...
***** The =my-emacs-notmuch.el= section about email composition settings
:PROPERTIES:
...
=message.el= ([[#h:49815c07-737f-4f10-95ee-63640f9ae18b][The =my-emacs-email.el= message composition and encryption settings (=message.el=)]]).
...
***** The =my-emacs-notmuch.el= section about reading messages
:PROPERTIES:
...
the built-in Simple HTML Renderer ([[#h:60483672-aeb6-427b-bbef-54a5c31e39ab][The =my-emacs-web.el= settings about the Simple HTML Renderer (~shr~)]]).
...
***** The =my-emacs-notmuch.el= section about hooks and key bindings
:PROPERTIES:
...
- Remind me when I am mentioning an attachment but do not include one.
  ...
  the ~notmuch-mua-attachment-regexp~ ([[#h:b91c062d-add7-42e6-b425-4fa287763b1b][The =my-emacs-notmuch.el= section about email composition settings]]).
...
- Do not activate the ~notmuch-hl-line-mode~ because I want the
  ...
  action ([[#h:bf5b4d08-8f33-4a8c-8ecd-fca19bf2497a][The =my-emacs-theme.el= section for ~lin~]]).
...
- Define key bindings that make sense to me. The most important change
  ...
  in the section about my custom extensions for Notmuch ([[#h:dbd2b775-8f51-451a-8e24-9c8424fd2e89][The =my-emacs-notmuch.el= custom extensions (per =my-notmuch.el=)]]).
...
***** The =my-emacs-notmuch.el= custom extensions (per =my-notmuch.el=)
:PROPERTIES:
:CUSTOM_ID: h:dbd2b775-8f51-451a-8e24-9c8424fd2e89
:END:

My =my-notmuch.el= extension defines a few useful extras for my
email setup ([[#h:92ee86b8-0ead-4080-963f-3fbc9f931e3e][The =my-notmuch.el= library]]). These are commands to
...
those that are for the base Notmuch package ([[#h:6148eea6-7859-4b53-95ce-7e198fc831e0][The =my-emacs-notmuch.el= section about hooks and key bindings]]).
...
***** The =my-emacs-notmuch.el= glue code for ~org-capture~ (=ol-notmuch.el=)
:PROPERTIES:
...
***** The =my-emacs-notmuch.el= section about the ~notmuch-indicator~
:PROPERTIES:
...
***** The =my-emacs-notmuch.el= call to ~provide~
:PROPERTIES:
:CUSTOM_ID: h:340e745d-544c-4473-969e-ade21ae7710d
:END:

Finally, we ~provide~ the submodule. This is the mirror function of
~require~ ([[#h:200bc618-678a-4dc8-9d30-876d89e1edae][The =my-emacs-email.el= loading of the email client and call to ~provide~]]).

#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-notmuch.el"
(provide 'my-emacs-notmuch)
#+end_src

**** The deprecated =my-emacs-mail.el= submodule for ~mu4e~ (=my-emacs-mu4e.el=)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-mu4e.el" :mkdirp yes
...
**** The deprecated =my-emacs-mail.el= submodule for Gnus (=my-emacs-gnus.el=)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-gnus.el" :mkdirp yes
...
as ~mu4e~ ([[#h:97282c11-475c-4ad3-9399-cb2cd12f93b7][The deprecated =my-emacs-mail.el= submodule for ~mu4e~ (=my-emacs-mu4e.el=)]]), while the
latter's is ~notmuch~ or =notmuch.el= to differentiate it from the
system program ([[#h:b26c6297-ebe8-4f99-b069-df53cfd90520][The =my-emacs-email.el= submodule for ~notmuch~ (=my-emacs-notmuch.el=)]]).
...
implements, which does not rely on ~msmtp~ ([[#h:c7c9815c-e259-4810-816a-9ebc1a146004][The =my-emacs-email.el= settings for ~sendmail~ (SMTP)]]).
...
daily computing with the rest of what I do ([[#h:755e195b-9471-48c7-963b-33055969b4e2][The =my-emacs-email.el= module]]).
...
file ([[#h:24f17614-ee3d-4cd1-bbe3-f53e4f39c281][The =my-emacs-email.el= basic settings (including =authinfo=)]]).
...
*** TODO The =my-emacs-web.el= module
:PROPERTIES:
:CUSTOM_ID: h:f23d9cef-ab7b-4486-a070-9c7ae664eadf
:END:

**** The =my-emacs-web.el= settings about following links (~browse-url~)
:PROPERTIES:
...
browser ([[#h:b09b4fce-31fe-4381-a49a-6a7206ac5f1b][The =my-emacs-web.el= settings about the web browser (~eww~)]]).
Emacs it can render HTML well, although it does not handle CSS and
JavaScript ([[#h:60483672-aeb6-427b-bbef-54a5c31e39ab][The =my-emacs-web.el= settings about the Simple HTML Renderer (~shr~)]]).
...
- [[#h:755e195b-9471-48c7-963b-33055969b4e2][The =my-emacs-email.el= module]]
- [[#h:adc258d3-f444-4bb5-8a8d-43025a689167][The =my-emacs-web.el= RSS/Atom reader (~elfeed~)]]
- [[#h:b9e6834b-e636-414e-8c50-9e04ba77a0a3][The =my-emacs-web.el= settings for the IRC client]]

#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-web.el" :mkdirp yes
;;;; `browse-url'
...
**** The =my-emacs-web.el= settings about buttonising links (~goto-address-mode~)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-web.el"
;;;; `goto-addr'
...
**** The =my-emacs-web.el= settings about the Simple HTML Renderer (~shr~)
:PROPERTIES:
...
- [[#h:edb70963-d590-4959-bc59-1e5435c8e78a][The =my-emacs-web.el= settings about following links (~browse-url~)]]
- [[#h:7639683b-4b6f-42ee-9ae0-76eeec6fee9a][The =my-emacs-web.el= settings about buttonising links (~goto-addr~)]]
- [[#h:b09b4fce-31fe-4381-a49a-6a7206ac5f1b][The =my-emacs-web.el= settings about the web browser (~eww~)]]
...
buffer (as I do, for example, via the ~text-mode-hook~ ([[#h:60d6aae2-6e4b-402c-b6a8-411fc49a6857][The =my-emacs-theme.el= section about ~variable-pitch-mode~ and font resizing]])).
...
**** The =my-emacs-web.el= settings about browser cookies
:PROPERTIES:
...
**** The =my-emacs-web.el= settings about the web browser (~eww~)
:PROPERTIES:
...
page with ~shr~ ([[#h:60483672-aeb6-427b-bbef-54a5c31e39ab][The =my-emacs-web.el= settings about the Simple HTML Renderer (~shr~)]]).
...
like Firefox ([[#h:edb70963-d590-4959-bc59-1e5435c8e78a][The =my-emacs-web.el= settings about following links (~browse-url~)]]).
...
**** The =my-emacs-web.el= extras for ~eww~ (=my-eww.el=)
:PROPERTIES:
:CUSTOM_ID: h:1fcd7008-b7ed-4441-8a7b-82db43a05775
:END:

My =my-eww.el= used to contain lots of tweaks for ~eww~, however
many of those are now built into Emacs. So I have trimmed down my
file to provide only these commands:

- ~my-eww-visit-url-on-page~ :: Use minibuffer completion to select
...
- ~my-eww-find-feed~ :: Produce a bespoke buffer listing the
  RSS/Atom feeds provided by the current website.

- ~my-eww-open-in-other-window~ :: Like ~eww~, but operate in a new
...
**** TODO The =my-emacs-web.el= RSS/Atom reader (~elfeed~)
:PROPERTIES:
...
**** The =my-emacs-web.el= settings for the IRC client (~rcirc~)
:PROPERTIES:
...
function ~my-common-auth-get-field~ to extract a password from my
=.authinfo.gpg= file ([[#h:24f17614-ee3d-4cd1-bbe3-f53e4f39c281][The =my-emacs-email.el= basic settings (including =authinfo=)]]).
...
*** The =my-emacs-which-key.el= module
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-which-key.el" :mkdirp yes
...
*** The =my-emacs-icons.el= module (~nerd-icons~ for various packages)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-icons.el" :mkdirp yes
;;; Icons
...
*** The =my-emacs-pdf.el= module
:PROPERTIES:
:ID:       efe1e062-5499-49d0-95e9-01402592a615
:END:

I use Emacs a lot to read books

#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-pdf.el"
...
*** The =my-emacs-gptel.el= module

#+begin_src emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-emacs-gptel.el"
(use-package gptel
  :ensure t
  :config
  (setq gptel-backend
...
#+end_src
*** The =my-functions.el= for exwm
**** Functions

Some straightforward functions here, used to make my Emacs life a little easier.
#+BEGIN_SRC emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs.d/my-emacs-modules/my-functions.el"
...
*** The =my-abbrev.el= library
:PROPERTIES:
:CUSTOM_ID: h:ddb9c32c-2114-4347-9f76-5160e590b5dc
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-abbrev.el" :mkdirp yes
...
*** The =my-comment.el= library
:PROPERTIES:
:CUSTOM_ID: h:7affcbca-0808-4159-8eab-faa7973aefc7
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-comment.el" :mkdirp yes
...
*** The =my-common.el= library
:PROPERTIES:
:CUSTOM_ID: h:3fccfadf-22e9-457f-b9fd-ed1b48600d23
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-common.el" :mkdirp yes
...
*** The =my-dired.el= library
:PROPERTIES:
:CUSTOM_ID: h:92cfefcf-8c20-43cd-b47c-ffe0c5fe1887
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-dired.el" :mkdirp yes
...
*** The =my-elfeed.el= library
:PROPERTIES:
:CUSTOM_ID: h:98c313d7-62c2-48c1-b6ad-28cc25f22092
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-elfeed.el" :mkdirp yes
...
*** The =my-eww.el= library
:PROPERTIES:
:CUSTOM_ID: h:abc20037-7a4f-4555-809a-dc4165c5db6a
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-eww.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "my-lisp/my-modeline.el" :mkdirp yes
...
*** The =my-notmuch.el= library
:PROPERTIES:
:CUSTOM_ID: h:92ee86b8-0ead-4080-963f-3fbc9f931e3e
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-notmuch.el" :mkdirp yes
...
*** The =my-orderless.el= library
:PROPERTIES:
:CUSTOM_ID: h:114c6e8c-697a-43e9-8fd9-52872997e1c9
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-orderless.el" :mkdirp yes
...
*** The =my-org.el= library
:PROPERTIES:
:CUSTOM_ID: h:9974eac8-2167-45c4-90e0-12dd877403da
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-org.el" :mkdirp yes
...
*** The =my-pair.el= library
:PROPERTIES:
:CUSTOM_ID: h:0a3f076d-386d-47e6-8af9-f17d02d507cf
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-pair.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "my-lisp/my-prefix.el" :mkdirp yes
...
*** The =my-project.el= library
:PROPERTIES:
:CUSTOM_ID: h:654cfe49-f59d-4a0d-84ee-094ddce06848
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-project.el" :mkdirp yes
...
*** The =my-scratch.el= library
:PROPERTIES:
:CUSTOM_ID: h:efebaec4-7273-4c5a-a4c4-9f99df0e46bb
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-scratch.el" :mkdirp yes
...
*** The =my-search.el= library
:PROPERTIES:
:CUSTOM_ID: h:0cab525d-fcbc-478c-b914-bbd69e33c635
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-search.el" :mkdirp yes
...
*** The =my-shell.el= library
:PROPERTIES:
:CUSTOM_ID: h:32f6fe0f-23c4-44cc-97cc-3e5372bd484e
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-shell.el" :mkdirp yes
...
*** The =my-simple.el= library
:PROPERTIES:
:CUSTOM_ID: h:5f78e837-0d27-4390-bd9a-6d0bca57fa50
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-simple.el" :mkdirp yes
...
*** The =my-spell.el= library
:PROPERTIES:
:CUSTOM_ID: h:551718b7-fb2d-4a4a-804e-3118001b96ec
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-spell.el" :mkdirp yes
...
*** The =my-vertico.el= library
:PROPERTIES:
:CUSTOM_ID: h:3796a4c9-8659-4782-8aaa-3cf4e950927d
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-vertico.el" :mkdirp yes
...
*** The =my-window.el= library
:PROPERTIES:
:CUSTOM_ID: h:35b8a0a5-c447-4301-a404-bc274596238d
:END:

#+begin_src emacs-lisp :tangle "my-lisp/my-window.el" :mkdirp yes
...
because I can at any moment stop editing this =my-emacs.org= file
...
the ~outshine~ package ([[#h:ffff5f7b-a62b-4d4a-ae29-af75402e5c35][The =my-emacs-langs.el= settings for ~outline-minor-mode~]]).
...
Furthermore, I can use the ~consult-outline~ or ~my-search-outline~
command to navigate to a heading with minibuffer completion.

What the =my-emacs.org= provides is an /optional single point of
...
The =my-emacs.org= *does not introduce a dependency on Org mode*.
This file is not loaded at startup. I can remove =my-emacs.org= at
...
Ensure the ~exwm~ package is installed first.
