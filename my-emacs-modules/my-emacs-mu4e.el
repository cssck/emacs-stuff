...
(use-package mu4e
  ...
  (setq mu4e-compose-signature "Yurii Skoryk\n")
  ...
  (setq mu4e-contexts
        `(,@(mapcar
             (lambda (context)
               (let ((address (my-common-auth-get-field context :user)))
                 (make-mu4e-context
                  :name context
  ...
  (defun my/mu4e (&rest args)
    (cl-letf (((symbol-function 'display-buffer-full-frame) #'display-buffer-same-window))
      (apply args)))

  (advice-add #'mu4e-display-buffer :around #'my/mu4e)

  (defun my/mu4e-no-header-line ()
    (setq-local header-line-format nil))

  (add-hook 'mu4e-headers-mode-hook #'my/mu4e-no-header-line)

  (my-emacs-keybind global-map
    "C-c m" #'mu4e
    "C-x m" #'mu4e-compose-new) ; override `compose-mail'

  (my-emacs-keybind mu4e-headers-mode-map
  ...
  (my-emacs-keybind mu4e-view-mode-map
