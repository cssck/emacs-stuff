...
(use-package appt
  ...
  (with-eval-after-load 'org-agenda
    ...
    ;; NOTE 2021-12-07: In my `my-org.el' (see further below), I add
...
(use-package org
  ...
  ( :map global-map
    ...
    :map ctl-x-x-map
    ("i" . my-org-id-headlines)
    ("r" . my-org-id-headlines-readable)
    ("h" . my-org-ox-html))
  :config
...
(use-package org
  :ensure nil
  :config
  (require 'my-org) ; for the above commands
  ...
  (setq org-capture-templates
        `(("u" "Unprocessed" entry
          ...
          ("s" "Select file and heading to add to" entry
           (function my-org-select-heading-in-file)
           ,(concat "* TODO %^{Title}%?\n"
                    ":PROPERTIES:\n"
                    ":CAPTURED: %U\n"
                    ":CUSTOM_ID: h:%(format-time-string \"%Y%m%dT%H%M%S\")\n"
...
(use-package org-agenda
  ...
  ( :map global-map
    ("C-c A" . org-agenda)
    ("C-c a" . (lambda ()
                 "Call Org agenda with `my-org-custom-daily-agenda' configuration."
                 (interactive)
                 (org-agenda nil "A"))))
  :config
;;;;; Custom agenda blocks
  ...
  ;; Check the variable `my-org-custom-daily-agenda' in my-org.el
