...
(use-package notmuch
  :defer t
  :config
  (let ((prv (my-common-auth-get-field "prv-gandi" :user))
        (pub (my-common-auth-get-field "pub-gandi" :user))
        (inf (my-common-auth-get-field "inf-gandi" :user))
        (box (my-common-auth-get-field "my-gandi" :user)))
...
(use-package notmuch
  ...
  ;; Also see `notmuch-tagging-keys' in the `my-notmuch' section
  ;; further below.
  ;;
...
that and instead tries to complete against dictionary entries."
  ...
  (advice-add #'message-tab :override #'my-notmuch-message-tab))
...
;;; My own tweaks for notmuch (my-notmuch.el)
(use-package my-notmuch
  :ensure nil
  :after notmuch
  :bind
  ( :map notmuch-search-mode-map
    ("D" . my-notmuch-search-delete-thread)
    ("S" . my-notmuch-search-spam-thread)
    ("g" . my-notmuch-refresh-buffer)
    :map notmuch-show-mode-map
    ("D" . my-notmuch-show-delete-message)
    ("S" . my-notmuch-show-spam-message)
    :map notmuch-show-stash-map
    ("S" . my-notmuch-stash-sourcehut-link))
  :config
  ;; Those are for the actions that are available after pressing 'k'
  ;; (`notmuch-tag-jump').  For direct actions, refer to the key
  ;; bindings below.
  (setq notmuch-tagging-keys
        `((,(kbd "d") my-notmuch-mark-delete-tags "💥 Mark for deletion")
          (,(kbd "f") my-notmuch-mark-flag-tags "🚩 Flag as important")
          (,(kbd "s") my-notmuch-mark-spam-tags "🔥 Mark as spam")
...
(use-package notmuch-indicator
  ...
  (setq notmuch-indicator-args
        '(( :terms "tag:unread and tag:inbox"
            ;; :label "[U] "
            :label "💬 "
            :label-face my-modeline-indicator-cyan
            :counter-face my-modeline-indicator-cyan)
          ( :terms "tag:unread and tag:package"
            ;; :label "[P] "
            :label "🛠️ "
            :label-face my-modeline-indicator-magenta
            :counter-face my-modeline-indicator-magenta)
          ( :terms "tag:unread and tag:coach"
            ;; :label "[C] "
            :label "🏆 "
            :label-face my-modeline-indicator-red
            :counter-face my-modeline-indicator-red))

        notmuch-indicator-refresh-count (* 60 3)
        notmuch-indicator-hide-empty-counters t
        notmuch-indicator-force-refresh-commands '(notmuch-refresh-this-buffer))

  ;; I control its placement myself.  See my-emacs-modeline.el where
  ;; I set the `mode-line-format'.
  (setq notmuch-indicator-add-to-mode-line-misc-info nil)

  (notmuch-indicator-mode 1))

(provide 'my-emacs-notmuch)
