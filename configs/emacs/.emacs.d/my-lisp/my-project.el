;;; my-project.el --- Extensions for project.el -*- lexical-binding: t -*-
...
(defun my-project--switch (directory &optional command)
...
(defun my-project--frame-names ()
  "Return a list of frame names."
  (mapcar #'car (make-frame-names-alist)))

(defvar my-project-switch-hook nil
  "Normal hook called after `my-project-switch'.")

;;;###autoload
(defun my-project-switch (directory)
...
the project in DIRECTORY using `project-dired'."
  ...
  (let ((name (file-name-nondirectory (directory-file-name directory))))
    (if (member name (my-project--frame-names))
        (select-frame-by-name name)
      (my-project--switch directory 'project-dired))
    (run-hooks 'my-project-switch-hook)
    (setq this-command 'project-switch-project)))

;;;; Produce a VC root log for the project

(defun my-project-rename-vc-root-log (&rest _)
...
(advice-add #'vc-print-root-log :after #'my-project-rename-vc-root-log)
...
(defun my-project-in-tab--get-tab-names (&optional frame)
...
(defun my-project-in-tab--create-tab (directory name)
  "Create new tab visiting DIRECTORY and named NAME."
  (tab-new)
  (find-file directory)
  (unwind-protect
      (my-project--switch directory)
    (tab-rename name)
...
;;;###autoload
(defun my-project-in-tab (directory)
...
Use this as an alternative to `project-switch-project'."
  ...
  (project--remember-dir directory)
  (let ((name (file-name-nondirectory (directory-file-name directory))))
    (if (member name (my-project-in-tab--get-tab-names))
        (tab-switch name)
      (my-project-in-tab--create-tab directory name))))

;;;; Set up a project root

;; I don't actually have a use-case for `my-project-find-root',
;; but I wrote it once so I keep it here in case I ever need it.
;; Use it like this: (my-project-find-root c-mode "Makefile")
(defmacro my-project-find-root (mode file)
...
is a string."
  (let ((project-find-fn (intern (format "project-find-%s-root" mode)))
        (major-mode-fn (intern (format "my-%s-project-find-function" mode)))
    ...
    `(progn
       ...
       (defun ,(intern (format "my-%s-project-find-function" mode)) ()
