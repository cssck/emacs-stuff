...
(use-package my-common
  :ensure nil
  :functions (my-common-truncate-lines-silently)
  :hook ((text-mode prog-mode dired-mode my/fundamental-mode hexl-mode comint-mode) . my-common-truncate-lines-silently)
  :init
  ...
  (advice-add #'execute-extended-command--describe-binding-msg :override #'my-common-ignore))

(use-package my-simple
  :ensure nil
  :demand t
  :config
  (setq my-simple-date-specifier "%F")
  (setq my-simple-time-specifier "%R %z")

  (advice-add #'save-buffers-kill-emacs :before #'my-simple-display-unsaved-buffers-on-exit)

  ;; All `my-simple-override-mode' does is activate a key map.
  ...
  (my-simple-override-mode 0)

  (with-eval-after-load 'pulsar
    (add-hook 'my-simple-file-to-register-jump-hook #'pulsar-recenter-center)
    (add-hook 'my-simple-file-to-register-jump-hook #'pulsar-reveal-entry))
  :bind
  ( :map my-simple-override-mode-map
    ("C-a" . my-simple-duplicate-line-or-region) ; "again" mnemonic, overrides `move-beginning-of-line'
    ("C-d" . my-simple-delete-line) ; overrides `delete-char'

    ("C-v" . my-simple-multi-line-below) ; overrides `scroll-up-command'
    ("<next>" . my-simple-multi-line-below) ; overrides `scroll-up-command'
    ("M-v" . my-simple-multi-line-above) ; overrides `scroll-down-command'
    ("<prior>" . my-simple-multi-line-above) ; overrides `scroll-down-command'

    :map global-map
    ("ESC ESC" . my-simple-keyboard-quit-dwim)
    ("C-g" . my-simple-keyboard-quit-dwim)
    ("C-M-SPC" . my-simple-mark-sexp)   ; will be overriden by `expreg' if tree-sitter is available
    ("C-," . my-simple-mark-sexp)   ; I also have `isearch-forward-symbol-at-point' on C-.
    ;; Commands for lines
    ("C-S-d" . my-simple-delete-line-backward)
    ("M-k" . my-simple-kill-line-backward)
    ("M-j" . delete-indentation)
    ("C-w" . my-simple-kill-region)
    ("M-w" . my-simple-kill-ring-save)

    ("C-S-w" . my-simple-copy-line)
    ("C-S-y" . my-simple-yank-replace-line-or-region)
    ("<C-return>" . my-simple-new-line-below)
    ("<C-S-return>" . my-simple-new-line-above)
    ("C-x x a" . my-simple-auto-fill-visual-line-mode) ; auto-fill/visual-line toggle
    ;; Commands for text insertion or manipulation
    ("C-=" . my-simple-insert-date)
    ("C-<" . my-simple-escape-url-dwim)
    ;; "C->" my-simple-insert-line-prefix-dwim
    ("M-Z" . my-simple-zap-to-char-backward)
    ;; Commands for object transposition
    ("C-S-p" . my-simple-move-above-dwim)
    ("C-S-n" . my-simple-move-below-dwim)
    ("C-t" . my-simple-transpose-chars)
    ("C-x C-t" . my-simple-transpose-lines)
    ("C-S-t" . my-simple-transpose-paragraphs)
    ("C-x M-t" . my-simple-transpose-sentences)
    ("C-M-t" . my-simple-transpose-sexps)
    ("M-t" . my-simple-transpose-words)
    ;; Commands for paragraphs
    ("M-Q" . my-simple-unfill-region-or-paragraph)
    ;; Commands for windows and pages
    ("C-x o" . my-simple-other-window)
    ("C-x n k" . my-simple-delete-page-delimiters)
    ("M-r" . rotate-windows) ; Emacs 31 override `move-to-window-line-top-bottom'
    ("M-S-r" . rotate-windows-back) ; Emacs 31
    ;; Commands for buffers
    ("<C-f2>" . my-simple-rename-file-and-buffer)
    ("C-x k" . my-simple-kill-buffer-current)
    ("C-x K" . kill-buffer) ; leaving this here to contrast with the above
    ("M-s b" . my-simple-buffers-major-mode)
    ("M-s v" . my-simple-buffers-vc-root)
    ;; Commands for files
    ("C-x r ." . my-simple-file-to-register)))

;;;; Scratch buffers per major mode (my-scratch.el)
(use-package my-scratch
  :ensure nil
  :bind ("C-c s" . my-scratch-buffer)
  :config
  (setq my-scratch-default-mode 'text-mode))

;;;; Insert character pairs (my-pair.el)
(use-package my-pair
  :ensure nil
  :bind
  (("C-'" . my-pair-insert)
...
;;;; Comments (my-comment.el)
(use-package my-comment
  :ensure nil
  :init
  ...
  (setq my-comment-comment-keywords '("TODO" "NOTE" "XXX" "REVIEW" "FIXME"))
  (setq my-comment-timestamp-format-concise "%F")
  (setq my-comment-timestamp-format-verbose "%F %T %z")
  :bind
  (("C-;" . my-comment)
   ("C-x C-;" . my-comment-timestamp-keyword)))

;;;; Prefix keymap (my-prefix.el)
(use-package my-prefix
  :ensure nil
  :bind-keymap
  (("<insert>" . my-prefix)
   ("C-z" . my-prefix)))
...
(use-package substitute
  ...
  ;; C-c s is occupied by `my-scratch-buffer'.
...
(when (and (treesit-available-p) my-emacs-treesitter-extras)
  (use-package expreg
    :ensure t
    :functions (my/expreg-expand my/expreg-expand-dwim)
    ;; There is also an `expreg-contract' command, though I have no use for it.
    :bind ("C-M-SPC" . my/expreg-expand-dwim) ; overrides `mark-sexp'
    :config
    (defun my/expreg-expand (n)
      "Expand to N syntactic units, defaulting to 1 if none is provided interactively."
      (interactive "p")
      (dotimes (_ n)
        (expreg-expand)))

    (defun my/expreg-expand-dwim ()
