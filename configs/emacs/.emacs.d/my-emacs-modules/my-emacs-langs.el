...
;;; Flyspell and my-spell.el (spell check)
...
(use-package my-spell
  :ensure nil
  :bind
  (("M-$" . my-spell-spell-dwim)
   ("C-M-$" . my-spell-change-dictionary)
   ("M-i" . my-spell-spell-dwim) ; override `tab-to-tab-stop'
   ("C-M-i" . my-spell-change-dictionary)) ; override `complete-symbol'
  :config
  (setq my-spell-dictionaries
        '(("EN English" . "en")
          ("EL Ελληνικά" . "el")
          ("FR Français" . "fr")
          ("ES Espanõl" . "es")))

  ;; Also check my-spell.el for what I am doing with
  ;; `my-spell-ispell-display-buffer'.  Then refer to the
...
(use-package flymake
  :ensure nil
  :preface
  (defvar my/flymake-mode-projects-path
    (file-name-as-directory (expand-file-name "Projects" "~/Git/"))
    "Path to my Git projects.")

  (defun my/flymake-mode-lexical-binding ()
    (when lexical-binding
      (flymake-mode 1)))

  (defun my/flymake-mode-in-my-projects ()
    (when-let* ((file (buffer-file-name))
                ((string-prefix-p my/flymake-mode-projects-path (expand-file-name file)))
                ((not (file-directory-p file)))
                ((file-regular-p file)))
      (add-hook 'find-file-hook #'my/flymake-mode-lexical-binding nil t)))

  (add-hook 'emacs-lisp-mode-hook #'my/flymake-mode-in-my-projects)
  :bind
  ( :map ctl-x-x-map
  ...
  ;; NOTE 2023-07-03: `my-modeline.el' actually defines the counters
